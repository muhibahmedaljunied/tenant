...
class AddAccountNumberToContactsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('contacts', function (Blueprint $table) {
            ...
            $table->foreign('account_number_customer')->references('account_number')->on('ac_masters')->onDelete('cascade');

            $table->bigInteger('account_number_supplier')->nullable()->after('account_number_customer'); // account_number foreign to ac_masters
            $table->foreign('account_number_supplier')->references('account_number')->on('ac_masters')->onDelete('cascade');
        });
    }
