function customizeExcelExport(replacedContent) {
    let buttons = $.fn.dataTable.defaults.buttons;
    buttons[buttons.findIndex((button) => button.extend === 'excel')].customizeData = function (
        data
    ) {
        replacedContent(data);
        return data;
    };
    return buttons;
}
...
$(document).ready(function () {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        buttons: customizeExcelExport(function (content) {
            if ($('#supplier_report_tbl').DataTable().columns(':visible').count() >= 7) {
                for (let i = 0; i < content.body.length; i++) {
                    let row = content.body[i];

                    row[1].replace(__currency_symbol, '');
                    row[2].replace(__currency_symbol, '');
                    row[3].replace(__currency_symbol, '');
                    row[4].replace(__currency_symbol, '');
                    row[5].replace(__currency_symbol, '');
                    row[6].replace(__currency_symbol, '');
                }
            }
            for (let index = 0; index < content.footer.length; index++) {
                if (index == 0) {
                    continue;
                }

                content.footer[index] = content.footer[index].replace(__currency_symbol, '');
            }
        }),
    ...
    var stock_report_cols = [
        ...
        {
            data: 'total_purchase_price_without_tax',
            name: 'total_purchase_price_without_tax',
            searchable: false,
        },
    ];
    ...
    if ($('th.stock_price').length) {
        stock_report_cols.push({
            ...
            name: 'stock_value_by_sale_price_without_tax',
            searchable: false,
            orderable: false,
        });
        stock_report_cols.push({
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        buttons: customizeExcelExport(function (content) {
            ...
            if ($('#stock_report_table').DataTable().columns(':visible').count() >= 13) {
                // console.log(content);

                for (let i = 0; i < content.body.length; i++) {
                    let row = content.body[i];
                    let __unit = $('#stock_report_table tbody tr')
                        .eq(i)
                        .find('[data-unit]')
                        .data('unit');
                    units.push(__unit);

                    row[4] = row[4].replace(__unit, '');
                    row[10] = row[10].replace(__unit, '');
                    row[11] = row[11].replace(__unit, '');
                    row[12] = row[12].replace(__unit, '');
                    row[13] = row[13].replace(__unit, '');

                    row[3].replace(__currency_symbol, '');
                    row[5].replace(__currency_symbol, '');
                    row[7].replace(__currency_symbol, '');
                    row[9].replace(__currency_symbol, '');
                }

                units.forEach(
                    (unit) =>
                        (content.footer[11] = content.footer[11].replace(new RegExp(unit, 'g'), ''))
                );
                units.forEach(
                    (unit) =>
                        (content.footer[4] = content.footer[4].replace(new RegExp(unit, 'g'), ''))
                );
            }
        }),
        ajax: {
        ...
        fnDrawCallback: function (oSettings) {
            ...
            var total_stock_price_curr = sum_table_col(
                $('#stock_report_table'),
                'total_stock_price_curr'
            );
            ...
            $('#footer_total_pp_without_tax').text(
                sum_table_col($('#stock_report_table'), 'total_purchase_price_without_tax')
            );
            $('#footer_total_sell_price_without_tax').text(
                sum_table_col($('#stock_report_table'), 'stock_value_by_sale_price_without_tax')
            );

            var total_potential_profit = sum_table_col(
                $('#stock_report_table'),
                'potential_profit'
            );
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        buttons: customizeExcelExport(function (content) {
            if ($('#product_sell_report_table').DataTable().columns(':visible').count() >= 13) {
                let units = [];

                for (let i = 0; i < content.body.length; i++) {
                    let row = content.body[i];
                    let __unit = $('#product_sell_report_table tbody tr')
                        .eq(i)
                        .find('[data-unit]')
                        .data('unit');
                    units.push(__unit);

                    row[7] = row[7].replace(__unit, '');
                    row[9].replace(__currency_symbol, '');
                    row[10].replace(__currency_symbol, '');
                    row[11].replace(__currency_symbol, '');
                    row[12].replace(__currency_symbol, '');
                }

                content.footer[11] = content.footer[11].replace(__currency_symbol, '');

                units.forEach(
                    (unit) =>
                        (content.footer[6] = content.footer[6].replace(new RegExp(unit, 'g'), ''))
                );
            }
        }),
    ...
    product_sell_report_with_purchase_table = $(
        ...
        buttons: customizeExcelExport(function (content) {
            if (
                ...
                for (let i = 0; i < content.body.length; i++) {
                    let row = content.body[i];
                    let __unit = $('table#product_sell_report_with_purchase_table tbody tr')
                        .eq(i)
                        .find('[data-unit]')
                        .data('unit');
                    row[7] = row[7].replace(__unit, '');
    ...
    product_sell_grouped_report = $('table#product_sell_grouped_report_table').DataTable({
        ...
        buttons: customizeExcelExport(function (content) {
            if (
                $('#product_sell_grouped_report_table').DataTable().columns(':visible').count() >=
                13
            ) {
                for (let i = 0; i < content.body.length; i++) {
                    let row = content.body[i];
                    let __unit = $('#product_sell_grouped_report_table tbody tr')
                        .eq(i)
                        .find('[data-unit]')
                        .data('unit');
                    row[3] = row[3].replace(__unit, '');
                    row[4] = row[4].replace(__unit, '');
                    row[5].replace(__currency_symbol, '');
                }
            }
    ...
    items_report_table = $('#items_report_table').DataTable({
        ...
        buttons: customizeExcelExport(function (content) {
            let units = [];
            if ($('#items_report_table').DataTable().columns(':visible').count() >= 13) {
                for (let i = 0; i < content.body.length; i++) {
                    let __unit = $('#items_report_table tbody tr')
                        .eq(i)
                        .find('[data-unit]')
                        .data('unit');
                    units.push(__unit);
            ...
            units.forEach(
                (unit) =>
                    (content.footer[10] = content.footer[10].replace(new RegExp(unit, 'g'), ''))
            );
            content.footer[11] = content.footer[11].replace(__currency_symbol, '');

            content.footer[12] = content.footer[12].replace(__currency_symbol, '');
            content.footer[5] = content.footer[5].replace(__currency_symbol, '');
        }),
...
function updateTaxPurchaseSellsReport() {
    var start = $('#tax_report_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD');
    var end = $('#tax_report_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
    var location_id = $('#tax_report_location_id').val();
    var data = { start_date: start, end_date: end, location_id: location_id };
    ...
    var loader = '<i class="fas fa-sync fa-spin fa-fw margin-bottom"></i>';
    $('.info-box-number').html(loader);
    $.ajax({
        method: 'GET',
        url: '/reports/tax-sell-purchase-report',
        dataType: 'json',
        data: data,
        success: function (data) {
            // ------------------------------------------------------------------------------ \\
            $('.total_purchase').html(
                __currency_trans_from_en(data.purchases.total_without_tax, true)
            );
            $('.total_purchase_inc_tax').html(
                __currency_trans_from_en(data.purchases.total_with_tax, true)
            );
            $('.total_tax_purchase').html(__currency_trans_from_en(data.purchases.total_tax, true));
            // ------------------------------------------------------------------------------ \\
            $('.total_sell').html(__currency_trans_from_en(data.sells.total_without_tax, true));
            $('.total_sell_inc_tax').html(
                __currency_trans_from_en(data.sells.total_with_tax, true)
            );
            $('.total_tax_sell').html(__currency_trans_from_en(data.sells.total_tax, true));
        },
    });
}
