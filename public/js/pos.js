$(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.success == 1) {
                            ...
                            reset_pos_form();
                            console.log(result.kitchen.kitchens);
                            if (result.kitchen.kitchens) {
                                pos_print_kitchen(result.kitchen.kitchens);
                            }
                            //Check if enabled or not
                            if (result.receipt.is_enabled) {
                                pos_print(result.receipt);
    ...
    $('table#pos_table').on('change', 'select.another_variation_id', function () {
        $(this).parents('tr').remove();
        pos_total_row();
...
function pos_print_kitchen(kitchenPrinters) {
    Array.from(kitchenPrinters).forEach((printer, index) => {
        if (printer.print_type != 'printer') {
            kitchenPrinters.remove(index);
        }
    });

    var content = {
        receipt: kitchenPrinters,
        type: 'print-kitchen-receipt',
    };
    // console.log(content, kitchenPrinters);
