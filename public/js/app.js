$(document).ready(function () {
    ...
    // console.log('wowwwwwwww_unit',units_table.columns);
    ...
    //Start: CRUD for store
    //Store table
    var stores_table = $('#store_table').DataTable({
        processing: true,
        serverSide: true,
        // ajax: '/stores',
        ajax: {
        ...
        columns: [
            ...
            { data: 'short_name', name: 'short_name' },
            { data: 'business_locations', name: 'business_locations' },
            { data: 'action', name: 'action' },
        ],
    });

    // console.log('wowwwwwwww_store',stores_table);

    $(document).on('submit', 'form#store_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
    ...
    $(document).on('click', 'button.edit_store_button', function () {
        $('div.store_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#store_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_store_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_store,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
...
$(document).ready(function () {
    $('a[data-bs-target^="#menu_"]').on('click', function (e) {
        const href = $(this).attr('href');

        // Only prevent default and toggle icon if href is '#'
        if (href === '#') {
            e.preventDefault();
