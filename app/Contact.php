...
class Contact extends Authenticatable
{
    ...
    public function account_number_customer_details()
    {
        return $this->belongsTo(\App\Models\AcMaster::class, 'account_number_customer', 'account_number');
    }
    /**
    ...
    public static function customersDropdown($business_id, $prepend_none = true, $append_id = true)
    {
        ...
        if ($append_id) {
            $all_contacts->select(
                ...
                DB::raw("CASE 
                WHEN contact_id IS NULL OR contact_id = '' 
                THEN ISNULL(supplier_business_name, '') + ' - ' + ISNULL(name, '') 
                ELSE ISNULL(supplier_business_name, '') + ' - ' + ISNULL(name, '') + ' (' + ISNULL(contact_id, '') + ')' 
             END AS customer")
     ,'id'
            );
        } else {
