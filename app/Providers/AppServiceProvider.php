...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // if (request()->has('lang')) {
        //     app()->setLocale(trim(request('lang')));
        // }
        if (request()->has('lang')) {
        ...
        if ($currentHost === $centralDomain) {
            ...
            View::composer(
                ['layouts.*'],
                function ($view) use ($moduleService) {
                    if (isAppInstalled()) {
                        $keys = ['additional_js', 'additional_css'];
                        $__system_settings = $moduleService->getProperties($keys, true);

                        //Get js,css from modules
                        $module_additional_script = $moduleService->getModuleData('get_additional_script');
                        $additional_views = [];
                        $additional_html = '';
