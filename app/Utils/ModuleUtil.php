...
{
    public static $activeSubscription = null;
    public static $isSuperAdminInstalled = null;
    public static $installedModules = null;

    public function getModuleStatus($module)
    {
        $filename = base_path("modules_statuses.json");
        $handler = fopen($filename, "r");
        $file = fread($handler, filesize($filename));
        fclose($handler);
        return json_decode($file)->{$module};
    }


    public function allInstalledModules()
    {

        if (!self::$installedModules) {
            self::$installedModules = System::get();
        }
        return self::$installedModules;
    }
    public function findModule($key)
    {

        $row = $this->allInstalledModules()->where('key', $key)->first();
    ...
    public function getProperties($keys, $pluck = false)
    {
        if ($pluck) {
            return $this->allInstalledModules()->whereIn('key', $keys)
                ->pluck('value', 'key');
        } else {
            return $this->allInstalledModules()->whereIn('key', $keys)
                ->toArray();
    ...
    public function isModulesInstalled(array $modules)
    {
        $modulesKeys = array_map(function ($module) {
            if (Module::has($module) && $this->getModuleStatus($module)) {
                return [
                    'name' => $module,
                    'version_name' => strtolower($module) . "_version",
                ];
            }
        }, $modules);
    ...
    {
        $is_available = Module::has($module);

        if ($is_available && $this->getModuleStatus($module)) {

            //Check if installed by checking the system table {module}_version
            $module_version = $this->findModule(strtolower($module) . '_version');
    ...
    public function isSuperadminInstalled()
    {
        if (!self::$isSuperAdminInstalled) {
            self::$isSuperAdminInstalled = $this->isModuleInstalled('Superadmin');
        }
        return self::$isSuperAdminInstalled;
    }
    ...
    public function getModuleData($function_name, $arguments = null)
    {
        ...
        $installed_modules = $this->isModulesInstalled($moduleNames);
    ...
    public function userHasActiveSubcription($business_id)
    {
        if (!self::$activeSubscription) {
            self::$activeSubscription = \Modules\Superadmin\Entities\Subscription::active_subscription($business_id);
        }
    ...
    public function hasThePermissionInSubscription($business_id, $permission, $callback_function = null)
    {
        if ($this->isSuperadminInstalled()) {
            ...
            $package = $this->userHasActiveSubcription($business_id);
     ...
     *
     * @return \Illuminate\Contracts\View\View|\Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\Http\Response|
     */
        ...
        $response_array = [
            ...
            'msg' => __(
                ...
                [
                    ...
                    'subscribe_url' => route('subscription-index')
    ...
    public function getModuleVersionInfo($module_name)
    {
        ...
        if ($is_available) {
            ...
            $module_version = $this->findModule(strtolower($module_name) . '_version');
