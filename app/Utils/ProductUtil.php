...
use App\Models\VariationUnitPrice;
...
class ProductUtil extends Util
{
    ...
    public function createSingleProductVariation(
        ...
        $this->product_variation = $product->product_variations()->create($product_variation_data);
        ...
        $this->variations = $this->product_variation->variations()->create($variation_data);
    ...
    public function createVariableProductVariations(
        ...
        foreach ($input_variations as $key => $value) {
            ...
            $this->product_variation = ProductVariation::create($product_variation_data);
            ...
            if (! empty($value['variations'])) {
                ...
                $this->variations = $this->product_variation->variations()->createMany($variation_data);
                ...
                foreach ($this->variations as $variation) {
                    ...
                    // ----------------------------muhib add this------------------------
                    // dd('ddddddd', $this->variations);
                    $this->variations = $variation;
                    $this->init_store_in_variation_location_details($product->id);
                    // ---------------------------------------------------
    ...
    // -----------------muhib add this function----------------------------------------

    public function init_store_in_variation_location_details($product_id)
    {

        $variation_location_d = VariationLocationDetails::where('variation_id', $this->variations->id)
            ->where('product_id', $product_id)
            ->where('product_variation_id', $this->product_variation->id)
            ->where('location_id', $this->location_id)
            ->first();
        if (empty($variation_location_d)) {
            $variation_location_d = new VariationLocationDetails();
            $variation_location_d->variation_id = $this->variations->id;
            $variation_location_d->product_id = $product_id;
            $variation_location_d->location_id = $this->location_id;
            $variation_location_d->store_id = $this->store_id;
            $variation_location_d->product_variation_id = $this->product_variation->id;
     ...
     * @return  
     */
    public function updateVariableProductVariations(
        $product_id,
    ...
    public function updateProductQuantity(
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            $variation = Variation::where('id', $variation_id)
                ->where('product_id', $product_id)
                ->with('parentVariation')
                ->first();

            if ($variation->parentVariation) {
                $qty_difference = ($this->num_uf(
                    $variation->equal_qty,
                    $number_format
                ) * $new_quantity) - $this->num_uf($old_quantity, $number_format);
            }

            $variationId = $variation->id;
            if ($variation->parentVariation) {
                $variationId = $variation->parentVariation->id;
            }
            //Add quantity in VariationLocationDetails

            $variation_location_d = VariationLocationDetails::where('variation_id', $variationId)
                ->where('product_id', $product_id)
            ...
            if (empty($variation_location_d)) {
                $variation_location_d = new VariationLocationDetails();
                $variation_location_d->variation_id = $variationId;
                $variation_location_d->product_id = $product_id;
        ...
        return $variation_location_d->qty_available ?? true;
    }
     ...
     * @return void
     */
    public function decreaseProductQuantityCombo($combo_details, $location_id, $store_id)
    {
     ...
     * @return Variation
     */
    public function getDetailsFromVariation(
        $variation_id,
        $business_id,
        $location_id = null,
        $check_qty = true
        ...
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
            ...
            ->leftjoin('variation_location_details AS P_VLD', 'variations.parent_variation_id', '=', 'P_VLD.variation_id')
            ...
            $query->where(function ($query) {
                $query->where(function ($q) {
                    $q
                        ...
                        ->whereNull('variations.parent_variation_id')
                        ->orWhere('vld.qty_available', '>', 0);
                })
                    ->orWhere(function ($q) {
                        $q->whereNotNull('variations.parent_variation_id')
                            ->where('p.enable_stock', '!=', 1)
                            ->orWhereRaw('variations.equal_qty <= P_VLD.qty_available')
                            ->where('P_VLD.qty_available', '>', 0);
                    });
            });
        }
                ...
                $query->where(function ($q) use ($location_id) {
                    ...
                    ->orWhere(function ($query) use ($location_id) {
                        $query
                            ...
                            ->whereRaw('variations.equal_qty <= P_VLD.qty_available')
        ...
        $product = $query->select(
            ...
            CASE 
                WHEN pv.is_dummy = 0 
                THEN CONCAT(p.name, ' (', pv.name, ':', variations.name, ')') 
                ELSE p.name 
            END AS product_name
        "),
            ...
            'VU.id          as v_unit_id',
            'VU.allow_decimal as vu_unit_allow_decimal',

            'variations.equal_qty as variation_equal_qty',
            'variations.parent_variation_id as parent_varId',
            ...
            DB::raw("(
                SELECT TOP 1 purchase_price_inc_tax
                FROM purchase_lines
        ...
        if ($product) {
            ...
            if (! empty($product->variation_equal_qty) && ! empty($product->parent_varId)) {
                $qty_in_stock = ($product->parent_v_qty / $product->variation_equal_qty);
            }

            $unit_allow_decimal = $product->vu_unit_allow_decimal ?? $product->unit_allow_decimal;

            $product->setAttribute('qty_available', $qty_in_stock);
            $product->setAttribute('unit_allow_decimal', $unit_allow_decimal);
    ...
    public function calculateInvoiceTotal(
        ...
        foreach ($products as $product) {
            if (empty($product['item_tax']) && ! empty($product['sell_line_id'])) {
                $sellLineTransaction = TransactionSellLine::find($product['sell_line_id']);
                $product['item_tax'] = $sellLineTransaction->item_tax;
            }
        ...
        // $output['total_inc_tax']   = $output['total_before_tax'] - $output['product_tax'];
        $output['total_before_tax'] = $output['total_before_tax'] - $output['product_tax'];
        /* End */

        //Calculate total

        $output['final_total'] = (($output['total_before_tax'] + $output['product_tax']) + $output['tax']) - $output['discount'];

        return $output;
    }
    public function remapPurchaseProductVariations(array $products)
    {
        $variationdIds = Arr::pluck($products, 'variation_id');
        $variations = Variation::leftjoin('variations as pv', 'variations.parent_variation_id', '=', 'pv.id')
            ->whereIn('variations.id', $variationdIds)->select(
                'pv.id as parent_id',
                'variations.equal_qty as equal_qty',
                'variations.id as current_id'
            )->get();
        foreach ($products as $key => $product) {
            $variation = $variations->where('current_id', $product['variation_id'])->first();
            if ($variation) {
                if (! empty($variation->equal_qty) && ! empty($variation->parent_id)) {
                    $products[$key]['variation_id'] = ($variation->parent_id ?? $variation->current_id);
                    $products[$key]['quantity'] = ($variation->equal_qty * $product['quantity']);
                    $products[$key]['main_quantity'] = $product['quantity'];
                    $products[$key]['related_variation_id'] = ($variation->current_id);
                }
            }
        }
        return $products;
    }
    public function remapSellProductReturnVariations(array $products)
    {
        $variationdIds = Arr::pluck($products, 'variation_id');
        $variations = Variation::leftjoin('variations as pv', 'variations.parent_variation_id', '=', 'pv.id')
            ->whereIn('variations.id', $variationdIds)->select(
    ...
    public function remapSellProductVariations(array $products): array
    {
        $variationdIds = Arr::pluck($products, 'variation_id');
        $variations = Variation::leftjoin('variations as pv', 'variations.parent_variation_id', '=', 'pv.id')
            ->whereIn('variations.id', $variationdIds)->select(
        ...
        foreach ($products as $key => $product) {
            ...
            if ($variation) {
                if (! empty($variation->equal_qty) && ! empty($variation->parent_id)) {
                    ...
                    $products[$key]['quantity'] = ($variation->equal_qty * $product['quantity']);
                    $products[$key]['main_quantity'] = $product['quantity'];
                    $products[$key]['equal_qty'] = $variation->equal_qty;
                    $products[$key]['related_variation_id'] = ($variation->current_id);
                }
            }
        }
    ...
    protected function unitRules()
    {
        return [
            'another_unit.*.unit_id' => 'required|numeric|exists:units,id',
            'another_unit.*.qty' => 'required|numeric',
            'another_unit.*.single_dpp' => 'required|numeric',
            'another_unit.*.profit' => 'required|numeric',
            'another_unit.*.single_dpp_inc_tax' => 'required|numeric',
            'another_unit.*.sku' => 'required|string',
            'another_unit.*.single_dsp' => 'required|numeric',
            'another_unit.*.single_dsp_inc_tax' => 'required|numeric',
        ];
    }
    /**
     * Save Product Units
     ...
     * @return bool
     */
    public function createOrUpdateMultiProductUnits($request, $product)
    {
        $anotherUnitRules = $this->unitRules();
        $currentVariationsIds = [];
        $variation = $product->variations()->whereNull('parent_variation_id')->first();
        foreach ($request->another_unit as $id => $values) {
            $validator = Validator::make([
                "another_unit.{$id}.unit_id" => $values['unit_id'],
                "another_unit.{$id}.qty" => $values['qty'],
                "another_unit.{$id}.single_dpp" => $values['single_dpp'],
                "another_unit.{$id}.profit_percent" => $values['profit_percent'],
                "another_unit.{$id}.single_dpp_inc_tax" => $values['single_dpp_inc_tax'],
                "another_unit.{$id}.sku" => $values['sku'],
                "another_unit.{$id}.single_dsp" => $values['single_dsp'],
                "another_unit.{$id}.single_dsp_inc_tax" => $values['single_dsp_inc_tax'],
            ], $anotherUnitRules);
            if (! $validator->fails()) {
                $savedVariation = Variation::updateOrCreate([
                    'id' => $values['variation_id'] ?? 0,
                    'product_id' => $product->id,
                    ...
                    'product_id' => $product->id,
                    'sub_sku' => $values['sku'],
                    'default_purchase_price' => $this->num_uf($values['single_dpp']),
                    'dpp_inc_tax' => $this->num_uf($values['single_dpp_inc_tax']),
                    'profit_percent' => $this->num_uf($this->num_uf($values['profit_percent'])),
                    'default_sell_price' => $this->num_uf($values['single_dsp']),
                    'sell_price_inc_tax' => $this->num_uf($values['single_dsp_inc_tax']),
                    'combo_variations' => null,
                    'product_variation_id' => $variation->product_variation_id,
                    'parent_variation_id' => $variation->id,
                    'unit_id' => $values['unit_id'],
                    'equal_qty' => $values['qty'],
                ]);
                $currentVariationsIds[] = $savedVariation->id;
            }
        }
        $currentVariationsIds[] = $variation->id;
        $this->deleteProductVariationChildUnits($currentVariationsIds, $product);
        return true;
    }
    /**
     * Delete Product Units
     */
    public function deleteProductVariationChildUnits(array $currentProductUnitVariation, $product)
    {
        if (! empty($currentProductUnitVariation)) {
            return Variation::query()->where('product_id', $product->id)
                ->whereNotIn('id', $currentProductUnitVariation)
                ->delete();
        }
        return false;
    }
     ...
     * @param string $string
     *
     * @return string generated sku (string)
     */
     ...
     * @return void
     */
    public function adjustProductStockForInvoice(
        $status_before,
        $transaction,
        $input,
        $uf_data = true
     ...
     * @return void
     */
    public function addSingleProductOpeningStock(
        $business_id,
        $store_id,
        $product,
        $input,
        ...
        foreach ($input as $key => $value) {
            ...
            if (array_key_exists($location_id, $locations)) {
                ...
                $patch_number = null;
                if (! empty($value['patch_number'])) {
                    $patch_number = $value['patch_number'];
                }
                ...
                if ($qty > 0) {
                    ...
                    $purchase_line->patch_number = $patch_number;
    ...
    private function newPurchasesPricesAfterDiscount($transaction, $purchaseLines)
    {
        $lines_formatted = collect($purchaseLines);
        if (! is_null($transaction['discount_amount']) && $transaction['discount_amount'] > 0) {
            $discountAmount = $transaction['discount_amount'];
            $totalWithTax = ($lines_formatted->sum(fn($item) => $item['purchase_price_inc_tax'] * $item['quantity']));
        ...
        $taxProductsAmount = collect($lines_formatted)->sum(fn($item) => $item['item_tax'] * $item['quantity']);
        return [
            'total_before_tax' => $transaction['total_before_tax'],
            'products_tax' => $taxProductsAmount,
        ];
    }
    ...
    public function createOrUpdatePurchaseLines(
        ...
        //add journal_entry hire

        $transaction->discount_percentage_per_product = $transaction['discount_amount'];
        if ($transaction['discount_type'] != 'percentage') {
            $totalProductsPricesIncTax = collect($input_data)->sum(fn($pl) => abs($pl['quantity']) * abs($pl['purchase_price_inc_tax']));
            if ($totalProductsPricesIncTax > 0) {
                $transaction->discount_percentage_per_product = ($transaction['discount_amount'] / $totalProductsPricesIncTax * 100);
            }
        }
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $old_qty = $this->num_f($purchase_line->main_qty);
                if (! empty($data['related_variation_id'])) {
                    $purchase_line->related_variation_id = $data['related_variation_id'];
                    $purchase_line->main_qty_purchased = $data['main_quantity'];
                }
            ...
            $purchase_line->patch_number = ! empty($data['patch_number']) ? $data['patch_number'] : null;
            ...
            //add journal_entry_details hire
        }

        // dd(3);
        if ($createAcJournal) {
            // dd(4);
            $this->purchaseAccountingService->storeCreatePurchase($transaction, $input_data);
            // dd(5);
        }
        // dd(6);
     ...
     *
     * @param PurchaseLine $purchase_line
    ...
    public function adjustStockOverSelling($transaction)
    {
        ...
        if ($transaction->status != 'received') {  //this with purchases ! muhib add this comment
            return false;
        }
        ...
        foreach ($transaction->purchase_lines as $purchase_line) {
            ...
            if ($purchase_line->product->enable_stock == 1) {
                ...
                $sell_line_purchase_lines =
                    ...
                    ->get();
                //  dd($sell_line_purchase_lines,$purchase_line->quantity_remaining);
    ...
    private function mapFilterdProduct($content)
    {
        return $content->map(function ($item) {
            $qtyAvailable = $item['qty_available'];
            if (isset($item['equal_qty'])) {
                $qtyAvailable = ($item['parent_v_qty'] / $item['equal_qty']);
            }
            return [
                'name' => $item['name'],
                'type' => $item['type'],
                ...
                // 'equal_qty'      => $item['equal_qty'] ?? 0,
                // 'sku'            => $item['sku'],
                'unit' => ($item['va_unit'] ?? $item['unit']),
            ];
        });
    }
    /**
    ...
    public function filterProduct(
        ...
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ...
            ->leftjoin('units as VU', 'variations.unit_id', '=', 'VU.id')
            // ->leftjoin('variations as PAV', 'variations.parent_variation_id', '=', 'PAV.id')
            ->leftjoin(
        ...
        if (! empty($search_term)) {
                ...
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('sku', $search_fields)) {
                        $query->orWhere('products.sku', 'like', '%' . $search_term . '%')
                            ->orWhere('products.sku2', 'like', '%' . $search_term . '%')
            ...
            if ($search_type == 'exact') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('sku', $search_fields)) {
                        $query
                            ->orWhere('products.sku', $search_term);
                    }
        ...
        if ($check_qty) {
            ...
            $query
                ->where(function ($query) {
                    $query->where(function ($query) {
                        $query->whereNull('variations.parent_variation_id')
        ...
        $query->groupBy(
            ...
            'variations.sub_sku',
            'U.short_name',
            'variations.parent_variation_id',
            'variations.equal_qty',
            'VU.short_name',
            'variations.equal_qty',
            'P_VLD.qty_available',
        ...
        $products = $query->orderByDesc('VLD.qty_available')->get();
        // dd(
        //     $products
        // );
        return $this->mapFilterdProduct($products);
    }
    ...
    public function getProductStockDetails($business_id, $filters, $for, $alert_qty = false)
    {
        ...
        $products = $query->select(
            ...
            DB::raw("(
                SELECT SUM(
                    CASE 
                        WHEN transactions.type = 'sell_transfer' 
                        THEN TSL.quantity 
                        ELSE 0 
                    END
                )
            ...
            'variations.default_sell_price as unit_price_without_tax',
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->where('variations.id', $variation_id)
            ->select(
                DB::raw("SUM(CASE WHEN t.type = 'purchase' THEN pl.quantity ELSE 0 END) AS total_purchase"),
                DB::raw("SUM(CASE WHEN t.type = 'purchase' THEN pl.quantity_returned ELSE 0 END) AS total_purchase_return"),
                DB::raw("SUM(pl.quantity_adjusted) AS total_adjusted"),
                DB::raw("SUM(CASE WHEN t.type = 'opening_stock' THEN pl.quantity ELSE 0 END) AS total_opening_stock"),
                DB::raw("SUM(CASE WHEN t.type = 'purchase_transfer' THEN pl.quantity ELSE 0 END) AS total_purchase_transfer"),
                DB::raw("SUM(CASE WHEN t.type = 'production_purchase' THEN pl.quantity ELSE 0 END) AS total_production_purchase"),
        ...
        $sell_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->where('variations.id', $variation_id)
            ->select(
                DB::raw("SUM(CASE WHEN t.type = 'sell' THEN sl.quantity ELSE 0 END) AS total_sold"),
                DB::raw("SUM(CASE WHEN t.type = 'sell' THEN sl.quantity_returned ELSE 0 END) AS total_sell_return"),
                DB::raw("SUM(CASE WHEN t.type = 'sell_transfer' THEN sl.quantity ELSE 0 END) AS total_sell_transfer"),
                DB::raw("SUM(CASE WHEN t.type = 'production_sell' THEN sl.quantity ELSE 0 END) AS production_sell"),
    ...
    {
        $types = ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'production_sell', 'purchase_return', 'sell_return'];
        ...
        if (DB::table('stocktacking_lines')->exists()) {
            $types[] = 'stocktacking';
            $stock_history->leftjoin(
                'inventory_transactions',
                'inventory_transactions.transaction_id',
                '=',
                'transactions.id'
            )->leftjoin(
                'stocktacking_lines as inv_p',
                'inv_p.inventory_transaction_id',
                '=',
                'inventory_transactions.id'
            )->orwhere(function ($q) use ($variation_id, $location_id) {
                $q->where('inv_p.variation_id', $variation_id)->where('transactions.location_id', $location_id);
            })->addSelect(
                'inv_p.curent_quantity as stock_m_current_quantity',
                'inv_p.new_quantity as stock_m_new_quantity'
            );
        }


        $stock_history = $stock_history->whereIn('transactions.type', $types)->get();
        ...
        foreach ($stock_history as $stock_line) {
            if ($stock_line->transaction_type == 'sell') {
            ...
            } elseif ($stock_line->transaction_type == 'stocktacking') {
                $diff = abs($stock_line->stock_m_new_quantity) - abs($stock_line->stock_m_current_quantity);
                ...
                if ($diff > 0) {
                    ...
                    $stock_history_array[] = [
                        ...
                        'stock' => $this->roundQuantity($stock),
                        'type' => 'stocktacking',
                        'type_label' => __('inventory::lang.increase_inv'),
                        'ref_no' => $stock_line->invoice_no,
                        'transaction_id' => $stock_line->transaction_id
                    ];
                } elseif ($diff < 0) {
                    ...
                    $stock_history_array[] = [
                        ...
                        'stock' => $this->roundQuantity($stock),
                        'type' => 'stocktacking',
                        'type_label' => __('inventory::lang.decrease_inv'),
                        'ref_no' => $stock_line->invoice_no,
                        'transaction_id' => $stock_line->transaction_id
                    ];
                }
