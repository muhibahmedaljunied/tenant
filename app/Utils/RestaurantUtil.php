...
class RestaurantUtil extends Util
{
    ...
    public function kitchensProductsPrinter($business_id, $transactionId,$invoice_layout_id)
    {
        $business_details = $this->businessUtil->getDetails($business_id);


        $query = Transaction::whereId($transactionId)->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->select('location_id', 'id','invoice_no','transaction_date')
            ->where('transactions.status', 'final');
        ...
        $transaction = $query->with(['sell_lines:id,quantity,product_id,transaction_id', 'sell_lines.product.kitchen.printer'])
            ->latest()
            ->firstOrFail();
        ...
        foreach ($transaction->sell_lines->where('product.kitchen_id', '!=', NULL) as $item) {
            if (! isset($kitchens[$item->product->kitchen->printer_id])) {
                $kitchens[$item->product->kitchen->printer_id] = [

                    'invoice_no_prefix' => $invoice_layout->invoice_no_prefix,
                    'invoice_no' => $transaction->invoice_no,
                    'date_label' => $invoice_layout->date_label,
                    'invoice_date' =>   $this->format_date($transaction->transaction_date, true, $business_details),
                    'table_qty_label' => $invoice_layout->table_qty_label,
                    'table_product_label' => $invoice_layout->table_product_label,
                    ...
                    'print_type' => 'printer',
                    'printer_config' => $this->businessUtil->printerConfig($business_id, $item->product->kitchen->printer)
                ];
            } else {
                $kitchens[$item->product->kitchen->printer_id]['content']['products'][] = [
                    'name' => $item->product->name,
                    'quantity' => $item->quantity,
                ];
            }
    ...
    public function getKitchenOrders($business_id, $filter = [], $kitchen_id = 0)
    {
        $query = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->leftjoin(
                'business_locations AS bl',
