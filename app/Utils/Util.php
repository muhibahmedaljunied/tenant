...
class Util
{
    ...
    public function num_uf($input_number, $currency_details = null)
    {
        ...
        // if (isset($kk)) {
        //     dd('eeeeee', str_replace($thousand_separator, '', $input_number));
        // }
    ...
    {
        return in_array($module, $this->allModulesEnabled());
    }
    ...
    public function generateReferenceNumber($type, $ref_count, $business_id = null, $default_prefix = null)
    {
        ...
        if (! in_array($type, ['contacts', 'business_location', 'username'])) {
            return $prefix . now()->year . '/' . $ref_digits;
        } else {
            return $prefix . $ref_digits;
        }
     ...
     * @param Variation $product = null
     *
     * @return array
     */
    public function getVariationMultiUnits($business_id, $unit_id, $return_main_unit_if_empty = false, $product)
    {
        $variations = Variation::where('product_id', $product->product_id)->select('id', 'parent_variation_id', 'default_sell_price', 'sell_price_inc_tax', 'equal_qty', 'unit_id')->with('unit', 'parentVariation')->get();
        ...
        $sub_units = [];
        $qtyAvailable = (float) $product->formatted_qty_available;
        ...
        if (count($variations) > 0) {
            foreach ($variations as $variation) {
                ...
                if ($variation->parent_variation_id) {
                    ...
                    $sub_units[$variation->id] = [
                        'name' => optional($variation->unit)->actual_name,
                        'parent_id' => $variation->parent_variation_id ?? null,
                        'allow_decimal' => $variation->unit->allow_decimal ?? false,
                    ];
                    $unitEqualQty = ($qtyAvailable > 0 && (($qtyAvailable / $variation->equal_qty))) ? $qtyAvailable / $variation->equal_qty : 0;
                    if ($variation->unit->allow_decimal) {

                        $sub_units[$variation->id]['qty'] = $unitEqualQty;
                        $sub_units[$variation->id]['is_available'] = $unitEqualQty > 0;
                    } else {

                        $unitEqualQty = (int) $unitEqualQty;
                        $sub_units[$variation->id]['qty'] = (int) $unitEqualQty;
                        $sub_units[$variation->id]['is_available'] = $unitEqualQty > 0;
                    }
                } else {
                    $sub_units[$variation->id] = [
                        ...
                        'parent_id' => null,
                        'qty' => 1,
                        'is_available' => $qtyAvailable > 0,
                        'allow_decimal' => $variation->unit->allow_decimal ?? 0,
                    ];
                }
            }
        }
    ...
    public function getContactDue($contact_id)
    {
        $contact_payments = Contact::where('contacts.id', $contact_id)
            ...
            ->select(
                DB::raw("SUM(CASE WHEN t.status = 'final' AND t.type = 'sell' THEN final_total ELSE 0 END) AS total_invoice"),
                DB::raw("SUM(CASE WHEN t.type = 'purchase' THEN final_total ELSE 0 END) AS total_purchase"),
                DB::raw("SUM(CASE WHEN t.status = 'final' AND t.type = 'sell' THEN ISNULL(tp.total_paid, 0) ELSE 0 END) AS total_paid"),
                DB::raw("SUM(CASE WHEN t.type = 'purchase' THEN ISNULL(tp.total_paid, 0) ELSE 0 END) AS purchase_paid"),
                DB::raw("SUM(CASE 
WHEN t.type = 'opening_balance' 
THEN final_total 
ELSE 0 
