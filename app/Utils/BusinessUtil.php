...
class BusinessUtil extends Util
{
    ...
    public function newBusinessDefaultResources($business_id, $user_id)
    {
        ...
        $this->createBusinessTax($business->id, $user->id);
        ...
        $createAccounts = new CreateAccountsTemplate($business_id);


        $business_settings = $createAccounts->setDefaultAccounts()->create();
        ...
        AcMaster::create([
            ...
            "account_level" => 5,
            "parent_acct_no" => $customers,
            "account_type" => "debtor",
            'business_id' => $business_id
        ]);
        Log::info(16);

        $customer['account_number_customer'] = $next_children;
        // --------------muhib add this-------------------
        $customer['mobile'] = '';
        // -------------------------
    ...
    /**
     * Store Tax For Business
     *
     * @return TaxRate
     */
    public function createBusinessTax($businessId, $userId)
    {
        return TaxRate::create([
            'name' => "ضريبة القيمه المضافه",
            'for_tax_group' => 0,
    ...
    public function createNewBusiness($business_details)
    {
        ...
        return Business::create($business_details);
    }
    ...
    {
        return Business::leftjoin('tax_rates AS TR', 'business.default_sales_tax', 'TR.id')
    ...
    {
        $business = Business::whereId($business_id)->first();
     ...
     * @return array
     */
    public function printerConfig($business_id, $printer)
    {
     ...
     *
     * @param int $business_id
     * @param mixed $edit_transaction_period
     *
     * @return array
     */
    public function editTransactionDateRange($business_id, $edit_transaction_period)
    {
        if (is_numeric($edit_transaction_period)) {
            return [
                'start' => today()->subDays($edit_transaction_period),
                'end' => today()
            ];
