...
{
    public SaleEntriesPosService $saleEntriesPosService;
    public $purchases;

    public function __construct()
    {
        $this->saleEntriesPosService = app(SaleEntriesPosService::class);
    }
    ...
    public function createSellTransaction(
        ...
        if (! isset($input['branch_cost_center_id'])) {
            $input['branch_cost_center_id'] = $ac_setting->branch_cost_center_id;
        }
        if (! isset($input['extra_cost_center_id'])) {
            $input['extra_cost_center_id'] = $ac_setting->extra_cost_center_id;
        }
        ...
        return Transaction::create([
            ...
            'branch_cost_center_id' => ! empty($input['branch_cost_center_id']) ? $input['branch_cost_center_id'] : null,
            'extra_cost_center_id' => ! empty($input['extra_cost_center_id']) ? $input['extra_cost_center_id'] : null,
        ]);
    }
    ...
    private function newInvoicePricesAfterDiscount($transaction, $sellLines)
    {
        $lines_formatted = collect($sellLines);
        if (! is_null($transaction['discount_amount']) && $transaction['discount_amount'] > 0) {
            $discountAmount = $transaction['discount_amount'];
            $totalWithTax = ($lines_formatted->sum(fn($item) => $item['unit_price_inc_tax'] * $item['quantity']));
            if ($transaction['discount_type'] == 'percentage') {
                $discountAmount = round(($totalWithTax * $transaction['discount_amount'] / 100), 2);
            }
            $newTotalWithDiscount = ($totalWithTax - $discountAmount);
            $newTotalTaxes = ($totalWithTax - $newTotalWithDiscount);
            $newTotalWithoutTax = $newTotalWithDiscount - $newTotalTaxes;

            return [
                'total_before_tax' => round($newTotalWithoutTax, 2),
                'products_tax' => round($newTotalTaxes, 2),
            ];
        }

        return [
            'total_before_tax' => $transaction['total_before_tax'],
            'products_tax' => collect($lines_formatted)->sum(fn($item) => $item['item_tax'] * $item['quantity']),
        ];
    }
    /**
     ...
     */
    public function createOrUpdateSellLines(
        $transaction,
        $products,
        $location_id,
        ...
        if ($transaction->status !== 'draft' && $transaction->type == 'sell') { // this for sell operation       !muhib add this comment
            ...
            //add journal_entry hire sale
            $saleEntriesService = (new SaleEntriesPosService)->storeAllEntryDetails($transaction);
            $entryJournalSales = $saleEntriesService['sales'];
            //add journal_entry hire inv
            $entryJournalInventory = $saleEntriesService['inventory'];
        }
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                if (! empty($product['related_variation_id'])) {
                    $line['related_variation_id'] = $product['related_variation_id'];
                    $line['unit_equal_qty'] = $product['equal_qty'];
                }
                ...
                $lines_formatted[] = (new TransactionSellLine($line))->load('variations');
            ...
            //add journal_entry hire for sale

            // $product_details = Product::find($product['product_id']);
            // ****

            $product_details = Product::with(['media'])
                ...
                ->select(
                    ...
                    // DB::raw('v.default_purchase_price as purchase_price_without_tax'),
                    // DB::raw('v.dpp_inc_tax as purchase_price_inc_tax'),
            ...
            $purchase_price = 0;
            $sale_price = 0;
            $purchase_price = $product_details->min_purchase_price;
            $sale_price = $product_details->min_price;
            if ($product_details->max_purchase_price != $product_details->min_purchase_price && $product_details->type == "variable") {
                $purchase_price = $product_details->min_purchase_price - $product_details->max_purchase_price;
            }
            if ($product_details->max_price != $product_details->min_price && $product_details->type == "variable") {
                $sale_price = $product_details->min_price - $product_details->max_price;
            }

            //add journal_entry hire for inv

            // $transaction_details = Transaction::findOrFail($transaction);
            if ($transaction->status !== 'draft') {
                if (array_key_exists('quantity', $line)) {
                    $productTotalPurchasePrice = $purchase_price * ($line['quantity']);
                } else {

                    if (! empty($product['transaction_sell_lines_id'])) {
                        $sellLineQty = TransactionSellLine::where('id', $product['transaction_sell_lines_id'])->value('quantity');
                        $uf_quantity = $uf_data ? $this->num_uf($sellLineQty) : $sellLineQty;
                        $productTotalPurchasePrice = $purchase_price * ($uf_quantity);
                    }
                }
            }
        }
        if ($transaction->type == 'sell') {
            ...
            AcJournalEntryDetail::create([
                'account_number' => $ac_setting->sold_goods_cost,
                'amount' => $totalProductsPurchasePrice,
                'ac_journal_entries_id' => $entryJournalInventory->id,
            ...
            AcJournalEntryDetail::create([
                ...
                'amount' => ($totalProductsPurchasePrice) * (-1),
                'ac_journal_entries_id' => $entryJournalInventory->id,
                'cost_cen_field_id' => $transaction->extra_cost_center_id,
            ]);
        }
        $newInvoicePrices = $this->newInvoicePricesAfterDiscount($transaction, $lines_formatted);
        // ---------------------------------------------------------------------------------------------------------------------------------------
        if ($transaction->status != 'draft' && $transaction->type == 'sell') {
            if ($contact_details) {
                //customerinventory 
                AcJournalEntryDetail::create([
                    'account_number' => $contact_details->account_number_customer,
                    'amount' => $transaction->final_total,
                    'ac_journal_entries_id' => $entryJournalSales->id,
                    'cost_cen_field_id' => $transaction->extra_cost_center_id,
                ]);
            }

            // ****
            $total_sum_sales += $sale_price;
            if ($transaction->total_before_tax !== null) {
                //sales_service_revenue
                AcJournalEntryDetail::create([
                    'account_number' => $ac_setting->sales_service_revenue,
                    'amount' => $newInvoicePrices['total_before_tax'],
                    'ac_journal_entries_id' => $entryJournalSales->id,
                    'cost_cen_field_id' => $transaction->extra_cost_center_id,
                ]);
            }
            //vat_due
            $totalInvoiceTaxAmount = abs($transaction->tax_amount + $newInvoicePrices['products_tax']);

            AcJournalEntryDetail::create([
                'account_number' => $ac_setting->vat_due,
                'amount' => $totalInvoiceTaxAmount,
                'ac_journal_entries_id' => $entryJournalSales->id,
                'cost_cen_field_id' => $transaction->extra_cost_center_id,
            ]);
        }
        ...
        if (! empty($edit_ids)) {
            ...
            $combo_delete_lines = TransactionSellLine::whereIn(
                'parent_sell_line_id',
                $deleted_lines
            )->where(
                'children_type',
     ...
     * @return boolean
     */
    public function createOrUpdatePaymentLines(
        $transaction,
        $payments,
        $business_id = null,
        $user_id = null,
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        $tp = TransactionPayment::whereId($payment_id)->first();
    ...
    protected function contactSetContactAddress($info, &$customerAddress)
    {
        if (empty($customerAddress) > 0) {
            return $customerAddress[] = "{$info}";
        }
        return $customerAddress[] = "<br>{$info}";
    }
    ...
    public function getReceiptDetails(
        ...
        if ($il->show_customer == 1) {
            ...
            $customerIgnoredValuesAsAddressInfo = ['supplier_business_name', 'name', 'mobile', 'landline'];
            $customerAddressValues = ['supplier_business_name', 'address_line_1', 'address_line_2', 'city', 'state', 'country', 'zip_code', 'name', 'mobile', 'landline'];
            //To be used in pdfs    
            $customer_address = [];
            $customer_full_address = [];

            foreach ($customerAddressValues as $value) {
                $customerValue = $customer->{$value};
                if (! empty($customerValue)) {
                    if (! in_array($value, $customerIgnoredValuesAsAddressInfo)) {
                        $this->contactSetContactAddress($customerValue, $customer_full_address);
                    }
                    $this->contactSetContactAddress($customerValue, $customer_address);
                }
            }
            ...
            if (! empty($customer_address)) {
                ...
                $output['customer_full_address'] = implode(', ', $customer_full_address);
            }
        }
        ...
        if ($il->show_commission_agent == 1) {
            ...
            $output['commission_agent'] = ! empty($transaction->commissionagent->user_full_name) ? $transaction->commissionagent->user_full_name : '';
            $output['commission_agent_id'] = ! empty($transaction->commissionagent->contact_id) ? $transaction->commissionagent->contact_id : '';
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $sell_line_relations[] = 'related_variations';
            $sell_line_relations[] = 'related_variations.unit';
            ...
            $discountPerItem = $this->discountAmountPerItem(
                count($details['lines']),
                $discount
            );
            // ------------------------- \\
            ...
            foreach ($details['lines'] as $line) {
                ...
                $total_line_taxes += $this->totalTaxAfterDiscount($line, $discountPerItem);
                // -------------------------------- \\
            }
        ...
        $output['total_lines_tax'] = $this->num_f($total_line_taxes, $show_currency, $business_details);
        $output['total_lines_tax_uf'] = $this->num_f($total_line_taxes, false, $business_details);
        ...
        $output['discount_uf'] = ($discount != 0) ? $this->num_f($discount, false, $business_details) : 0;
        // -------------------- \\
        $output['total_discount'] = $this->num_f(($total_line_discount + $discount), true, $business_details);
        $output['total_discount_uf'] = $this->num_f(($total_line_discount + $discount), false, $business_details);
        // -------------------- \\
        $output['total_taxes'] = $this->num_f(($total_line_taxes), $show_currency, $business_details);
        // -------------------- \\
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
            $output['total_unformatted'] = $this->num_f($transaction->final_total, false, $business_details);
        }
        ...
        $output['total_taxes_after_discount'] = $this->num_f($output['tax'], $show_currency, $business_details);
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            if (! $customer->is_default) {
                // -------------------------------------------------------------- \\
                $list_contact_payments = (new ContactUtil)->getContactAccountantDueDetails($customer->account_number_customer);
                // -------------------------------------------------------------- \\
                $balanceSheetDue = ($list_contact_payments['all']['total_invoice'] - $list_contact_payments['all']['total_paid']);
                $balanceSheet_PaidAmount = abs($list_contact_payments['all']['total_paid']);
                $balanceSheetTotalAmount = abs($list_contact_payments['all']['total_invoice']);
                // -------------------------------------------------------------- \\
                $output['balance_sheet'] = [
                    'before' => [
                        'unformatted' => ($balanceSheetDue == 0) ? 0 : $this->num_uf($balanceSheetDue, false),
                        'formatted' => ($balanceSheetDue == 0) ? 0 : $this->num_f($balanceSheetDue, $show_currency, $business_details),
                    ],
                    'total' => [
                        'unformatted' => ($balanceSheetTotalAmount == 0) ? 0 : $this->num_uf($balanceSheetTotalAmount, false),
                        'formatted' => ($balanceSheetTotalAmount == 0) ? 0 : $this->num_f($balanceSheetTotalAmount, $show_currency, $business_details),
                    ],
                    'paid_amount' => [
                        'unformatted' => ($balanceSheet_PaidAmount == 0) ? 0 : $this->num_uf($balanceSheet_PaidAmount, false),
                        'formatted' => ($balanceSheet_PaidAmount == 0) ? 0 : $this->num_f($balanceSheet_PaidAmount, $show_currency, $business_details),
                    ],
                ];
            }
    ...
    {
        $is_patch_number_enabled = $business_details->enable_patch_number;
        $is_lot_number_enabled = $business_details->enable_lot_number;
        ...
        foreach ($lines as $line) {
            ...
            if (! empty($line->related_variations->unit)) {
                $unit_name = $line->related_variations->unit->short_name;
            }
            ...
            $line_array = [
                ...
                'quantity' => $this->num_f($line->main_qty, false, $business_details, true),
                'quantity_uf' => $line->main_qty,
                'units' => $unit_name,
                ...
                'line_total_tax' => $this->num_f(($line->item_tax * $line->main_qty), false, $business_details),
                ...
                'price_exc_tax' => $line->main_qty * $line->unit_price,
                ...
                'line_total' => $this->num_f($line->unit_price_inc_tax * $line->main_qty, false, $business_details),
                'line_total_uf' => $line->unit_price_inc_tax * $line->main_qty,
                'line_total_exc_tax' => $this->num_f($line->unit_price * $line->main_qty, false, $business_details),
                'line_total_exc_tax_uf' => $line->unit_price * $line->main_qty
            ];
            ...
            if (! empty($tax_details)) {
                if ($tax_details->is_tax_group) {
                    $group_tax_details = $this->groupTaxDetails($tax_details, $line->main_qty * $line->item_tax);
            ...
            if ($is_patch_number_enabled == 1 && $il->show_patch_number == 1) {
                $line_array['patch_number'] = ! empty($line->lot_details->patch_number) ? $line->lot_details->patch_number : null;
                $line_array['patch_number_label'] = __('lang_v1.lot_number', [], 'en');
            }
            if ($is_product_expiry_enabled == 1 && $il->show_expiry == 1) {
                ...
                $line_array['product_expiry_label'] = __('lang_v1.expiry', [], 'en');
            }
    ...
    protected function _receiptDetailsSellReturnLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if (! empty($line->related_variations)) {
                $unit_name = $line->related_variations->unit->short_name;
            }
            ...
            $line_array = [
                ...
                'variation' => (empty($variation->name) || $variation->name == 'DUMMY') ? '' : $variation->name,
                //Field for 2nd column
                'quantity' => $this->num_f($line->returned_main_qty, false, $business_details, true),
                'units' => $unit_name,
                'quantity_uf' => $line->returned_main_qty,

                'unit_price' => $this->num_f($line->unit_price, false, $business_details),
                'tax' => $this->num_f($line->item_tax, false, $business_details),
                'tax_name' => ! empty($tax_details) ? $tax_details->name : null,
                'line_total_tax' => $this->num_f(($line->item_tax * $line->returned_main_qty), false, $business_details),
                'tax_unformatted' => $line->item_tax,
                ...
                'line_total_exc_tax' => $this->num_f($line->unit_price * $line->returned_main_qty, false, $business_details),
                //Fields for 4th column
                'line_total' => $this->num_f($line->unit_price_inc_tax * $line->returned_main_qty, false, $business_details),
            ];
            ...
            if (! empty($tax_details)) {
                if ($tax_details->is_tax_group) {
                    $group_tax_details = $this->groupTaxDetails($tax_details, $line->returned_main_qty * $line->item_tax);
    ...
    public function getPurchaseTotals(
        ...
        $query = Transaction::leftJoin('transaction_payments as tp', 'tp.transaction_id', '=', 'transactions.id')
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'purchase')
            ->whereBetween(DB::raw('CAST(transactions.transaction_date AS date)'), ['2025-01-01', '2025-12-31'])
            ->select([
        ...
        return [
            'total_purchase_inc_tax' => $purchase_details->final_total_sum,
            'total_purchase_exc_tax' => $purchase_details->total_before_tax_sum,
            'purchase_due' => ($purchase_details->final_total_sum - $purchase_details->total_paid),
            'total_shipping_charges' => $purchase_details->total_shipping_charges,
    ...
    /**
     * Gives the total  tax for a business within the date range passed
     *
     * @param int $business_id
     ...
     * @return float
     */
    public function getTotalSellPurchaseWithTax(
        $business_id,
        $start_date = null,
        $end_date = null,
        $location_id = null
        ...
        $query1 = Transaction::where('transactions.business_id', $business_id)
            ...
            ->select(
                ...
                SUM(
                  CASE 
                    WHEN type = 'purchase' THEN transactions.tax_amount 
                    ELSE -1 * transactions.tax_amount 
                  END
                ) AS transaction_tax
              "),

                DB::raw("SUM( PL.item_tax * (PL.quantity - PL.quantity_returned) ) as purchase_line_item_taxes"),
                DB::raw("SUM( transactions.final_total ) as final_total"),
                DB::raw("SUM( transactions.total_before_tax ) as purchases_total"),
            );

        //Calculate Sells With taxes
        $query2 = Transaction::where('transactions.business_id', $business_id)
            ...
            ->select(
                ...
                SUM(
                  CASE 
                    WHEN type = 'sell' THEN transactions.tax_amount 
                    ELSE -1 * transactions.tax_amount 
                  END
                ) AS transaction_tax
              "),
                ...
                DB::raw("SUM( tsl.item_tax * (tsl.quantity - tsl.quantity_returned) ) as product_tax"),
                DB::raw("SUM( transactions.final_total ) as final_total"),
                DB::raw("SUM( transactions.total_before_tax ) as sells_total"),
        ...
        if (! empty($start_date) && ! empty($end_date)) {
            $query1->whereBetween(DB::raw('CONVERT(date,transaction_date)'), [$start_date, $end_date]);
            $query2->whereBetween(DB::raw('CONVERT(date,transaction_date)'), [$start_date, $end_date]);
        }
        ...
        $purchases_tax_details = $query1->first();

        $sells_tax_details = $query2->first();

        return [
            'sells' => [
                'total_without_tax' => $sells_tax_details->sells_total,
                'total_tax' => +$sells_tax_details->product_tax + $sells_tax_details->transaction_tax,
                'total_with_tax' => $sells_tax_details->final_total,
            ],
            'purchases' => [
                'total_tax' => $purchases_tax_details->purchase_line_item_taxes + $purchases_tax_details->transaction_tax,
                'total_without_tax' => $purchases_tax_details->purchases_total,
                'total_with_tax' => $purchases_tax_details->final_total,
            ],
        ];
    }
    /**
    ...
    public function getSellsLast30Days($business_id, $group_by_location = false)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            ...
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->whereBetween(DB::raw('CONVERT(date,transactions.transaction_date)'), [now()->subDays(30), now()]);
        ...
        $query->select(
            DB::raw("FORMAT(transactions.transaction_date, '%Y-%m-%d') as date"),
            DB::raw("SUM( transactions.final_total - COALESCE(SR.final_total, 0) ) as total_sells")
        )
            ->groupBy(DB::raw('CONVERT(DATE, transactions.transaction_date)'), 'transactions.transaction_date');

        if ($group_by_location) {
            $query->addSelect('transactions.location_id');
            $query->groupBy('transactions.location_id', 'transactions.transaction_date');
        }
        $sells = $query->get();
    ...
    public function getSellsCurrentFy($business_id, $start, $end, $group_by_location = false)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            ...
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->whereBetween(DB::raw('CONVERT(date,transactions.transaction_date)'), [$start, $end]);
    ...
    public function getExpenseReport(
        ...
        if (! empty($filters['start_date']) && ! empty($filters['end_date'])) {
            $query->whereBetween(DB::raw('CONVERT(date,transaction_date)'), [
                $filters['start_date'],
                $filters['end_date']
            ]);
        }
        ...
        if ($type == 'by_category') {
            $expenses = $query->select(
                ...
                DB::raw("SUM( CASE  WHEN transactions.type='expense_refund'
                THEN  -1 * final_total
                ELSE  final_total 
                END ) as total_expense"),
                'ec.name as category'
            )
                ->groupBy('expense_category_id', 'name')
                ->get();
        } elseif ($type == 'total') {
            $expenses = $query->select(
                // DB::raw("SUM( IF(transactions.type='expense_refund', -1 * final_total, final_total) ) as total_expense")
                DB::raw("SUM( CASE THEN transactions.type='expense_refund'
                THEN  -1 * final_total
                ELSE final_total
                END ) as total_expense")
    ...
    public function mapPurchaseSell(
        ...
        foreach ($transaction_lines as $line) {
            ...
            if ($stop_selling_expired && empty($purchase_line_id)) {
                ...
                $expiry_date = today()->addDays($stop_before)->toDateString();
            ...
            if (! ($qty_selling == 0 || is_null($qty_selling))) {
                ...
                if (! $allow_overselling) {
                    ...
                    logger()->emergency($mismatch_error . ' Business: ' . $business_name . ' Location: ' . $location_name);
...
"));
    ...
    public function getTotalSellCommission(
        ...
        if (! empty($start_date) && ! empty($end_date)) {
            $query->whereBetween(DB::raw('CONVERT(date,t.transaction_date)'), [$start_date, $end_date]);
        }
    ...
    public function getTotalPaymentWithCommission(
        $business_id,
        $start_date = null,
        $end_date = null,
        $location_id = null,
        ...
        if (! empty($start_date) && ! empty($end_date)) {
            $query->whereBetween(DB::raw('CONVERT(date,paid_on)'), [$start_date, $end_date]);
        }
    ...
    public function getLotNumbersFromVariation($variation_id, $business_id, $location_id, $exclude_empty_lot = false)
    {
        ...
        $purchase_lines = $query->select('purchase_lines.id as purchase_line_id', 'lot_number', 'patch_number', 'purchase_lines.exp_date as exp_date', DB::raw('(purchase_lines.quantity - (purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned)) AS qty_available'))->get();
...
")
     ...
     * @return void
     */
    public function updateQuantitySoldFromSellLine($sell_line, $new_quantity, $old_quantity, $uf_number = true, &$updatedPurchasesLines = [])
    {
        ...
        if ($qty_difference != 0) {
            ...
            $sell_line_purchase_lines = TransactionSellLinesPurchaseLines::where('sell_line_id', $sell_line->id)->with('purchase_line')->get();
            ...
            foreach ($sell_line_purchase_lines as $tslpl) {
                ...
                if ($qty_difference > 0) {
                    if ($tslpl->qty_returned < $tslpl->quantity) {
                        ...
                        $purchase_line = $tslpl->purchase_line;
                        if ($qty_left_to_update <= $tspl_qty_left_to_return) {
                            ...
                            $tslpl->save();
                            $updatedPurchasesLines[$tslpl->id] = $qty_left_to_update;
                            ...
                            $tslpl->save();
                            $updatedPurchasesLines[$tslpl->id] = $tspl_qty_left_to_return;
                ...
                elseif ($qty_difference < 0) {
                    ...
                    if (! empty($purchase_line)) {
                        ...
                        if ($tspl_qty_to_return >= 0) {
                            ...
                            $purchase_line->save();
                            $updatedPurchasesLines[$tslpl->id] = 0;
    ...
    public function getTransactionTotals(
        ...
        if (in_array('purchase_return', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'purchase_return' THEN final_total ELSE 0 END) AS total_purchase_return_inc_tax"),
                DB::raw("SUM(CASE WHEN transactions.type = 'purchase_return' THEN total_before_tax ELSE 0 END) AS total_purchase_return_exc_tax")
        ...
        if (in_array('sell_return', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'sell_return' THEN final_total ELSE 0 END) AS total_sell_return_inc_tax"),

                DB::raw("SUM(CASE WHEN transactions.type = 'sell_return' THEN total_before_tax ELSE 0 END) AS total_sell_return_exc_tax")
            );
        }

        if (in_array('sell_transfer', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'sell_transfer' THEN shipping_charges ELSE 0 END) AS total_transfer_shipping_charges")
        ...
        if (in_array('expense', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'expense' THEN final_total ELSE 0 END) AS total_expense")

            );

            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'expense_refund' THEN final_total ELSE 0 END) AS total_expense_refund")
        ...
        if (in_array('payroll', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN transactions.type = 'payroll' THEN final_total ELSE 0 END) AS total_payroll")
            );
        }
        ...
        if (in_array('stock_adjustment', $transaction_types)) {
            $query->addSelect(
                ...
                DB::raw("SUM(CASE WHEN transactions.type = 'stock_adjustment' THEN final_total ELSE 0 END) AS total_adjustment"),
                DB::raw("SUM(CASE WHEN transactions.type = 'stock_adjustment' THEN total_amount_recovered ELSE 0 END) AS total_recovered")
  ...
  SUM(
    CASE 
      WHEN transactions.type = 'purchase' THEN 
        CASE 
          WHEN discount_type = 'percentage' THEN COALESCE(discount_amount, 0) * total_before_tax / 100
          ELSE COALESCE(discount_amount, 0)
        END
      ELSE 0
    END
  ) AS total_purchase_discount
")
  ...
  SUM(
    CASE 
      WHEN transactions.type = 'sell' AND transactions.status = 'final' THEN 
        CASE 
          WHEN discount_type = 'percentage' THEN COALESCE(discount_amount, 0) * total_before_tax / 100
          ELSE COALESCE(discount_amount, 0)
        END
...
"),
  ...
  SUM(
    CASE 
      WHEN transactions.type = 'sell' AND transactions.status = 'final' 
      THEN rp_redeemed_amount 
      ELSE 0 
    END
...
"),
  ...
  SUM(
    CASE 
      WHEN transactions.type = 'sell' AND transactions.status = 'final' 
      THEN round_off_amount 
      ELSE 0 
    END
  ) AS total_sell_round_off
")
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $gross_profit_obj = $query->select(
            DB::raw("
        SUM(
          (transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) * 
          (SL.unit_price_inc_tax - ISNULL(PL.purchase_price_inc_tax, v.default_purchase_price))
        ) AS gross_profit
      ")
        )
            ->first();
    ...
    {
        $this->purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->join(
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw("FORMAT(transactions.transaction_date, 'yyyy/MM/dd') AS sale_date"),
                DB::raw("COALESCE(u.surname, '') + ' ' + COALESCE(u.first_name, '') + ' ' + COALESCE(u.last_name, '') AS added_by"),
                DB::raw("(
                    SELECT SUM(CASE WHEN TP.is_return = 1 THEN -1 * TP.amount ELSE TP.amount END)
                ...
                DB::raw("(
                    ...
                    WHERE TP2.transaction_id = SR.id
                ) AS return_paid"),
                ...
                DB::raw("COALESCE(ss.surname, '') + ' ' + COALESCE(ss.first_name, '') + ' ' + COALESCE(ss.last_name, '') AS waiter"),
        ...
        if (session('business.zatca_phase') === Zatca::STATUS_PHASE2) {
            $sells->addSelect([
                'zatca_status' => ZatcaInvoice::whereColumn('transaction_id', "transactions.id")->select('sent_to_zatca_status')
            ]);
        }
        return $sells;
    }
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start)
            ->select(
                ...
                DB::raw("SUM( CASE WHEN type = 'purchase'
                then final_total 
                else 0
                END ) as total_purchase"),
                DB::raw("SUM( CASE WHEN  type = 'sell' AND status = 'final'
                THEN final_total
                 ELSE  0
                 END ) as total_invoice"),
                DB::raw("SUM( CASE WHEN  type = 'sell_return' 
                THEN  final_total 
                ELSE  0
                END) as total_sell_return"),
                DB::raw("SUM( CASE WHEN  type = 'purchase_return' 
                THEN final_total 
                ELSE 0
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs['paid_on'] = $request->input('paid_on', now()->toDateTimeString());
        if ($format_data) {
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        //Get parent sale
        $sell = Transaction::where('business_id', $business_id)->whereId($input['transaction_id'])
            ->with([
                'sell_lines',
                'sell_lines.sell_line_purchase_lines.purchase_line',
                'sell_lines.related_variations',
                'sell_lines.sub_unit'
            ])->firstOrFail();
        ...
        $sell_return_data = [
            ...
            'branch_cost_center_id' => $sell->branch_cost_center_id,
            'extra_cost_center_id' => $sell->extra_cost_center_id
        ];
        $sellLineIds = Arr::pluck($input['products'], 'sell_line_id');
        $sellLines = $sell->sell_lines->whereIn('id', $sellLineIds);

        $linesFormatted = $sellLines->map(function ($sellLine) use ($input) {
            $arrItem = head(Arr::where($input['products'], fn($item) => $item['sell_line_id'] == $sellLine->id));
            return [
                'quantity' => $arrItem['quantity'],
                'unit_price_inc_tax' => $arrItem['unit_price_inc_tax'],
                'item_tax' => $sellLine->item_tax,
            ];
        });

        $returnInvoicePrices = $this->newInvoicePricesAfterDiscount([
            'discount_amount' => $sell_return_data['discount_amount'],
            'discount_type' => $sell_return_data['discount_type'],
            'total_before_tax' => $sell_return_data['total_before_tax']
        ], $linesFormatted);
        ...
        if (empty($sell_return)) {
            ...
            // ------
            $sell_return_data['store_id'] = $sell->store_id;
            ...
            // ------add this muhib---
            $sell_return_data['essentials_duration'] = 0;
            // ----------
        ...
        // total_before_tax
        // products_tax
        $this->saleEntriesPosService->storeReturnTransactionsEntries(
            $sell_return,
            $sell,
            $invoice_total,
            $returnInvoicePrices,
        ...
        foreach ($product_lines as $key => $product_line) {
            $sellLineItem = $sell->sell_lines->where('id', $product_line['sell_line_id'])->first();
            $returns[$product_line['sell_line_id']] = $uf_number ? $this->num_uf($product_line['quantity']) : $product_line['quantity'];
            if ($sellLineItem && $sellLineItem->related_variations) {
                $mainVariationUnitQty = ($product_line['quantity'] * $sellLineItem->related_variations->equal_qty);
                $sellQty = $uf_number ? $this->num_uf($mainVariationUnitQty) : $mainVariationUnitQty;
                $returns[$product_line['sell_line_id']] = $sellQty;
            }
        }
        ...
        foreach ($sell->sell_lines as $sell_line) {
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                $this->updateQuantitySoldFromSellLine($sell_line, $quantity, $quantity_before, false, $updatedPurchasesIds);

                // Update quantity in variation location details
                $productUtil->updateProductQuantity(
                    $sell_return->location_id,
                    $sell_return->store_id,
        ...
        if (! empty($updatedPurchasesIds)) {
            $sellLinesPurchaseLines = $sell->sell_lines->flatMap->sell_line_purchase_lines;
            $sumPricesPurchases = $sellLinesPurchaseLines->whereIn('id', array_keys($updatedPurchasesIds))->sum(function ($item) use ($updatedPurchasesIds) {
                return $updatedPurchasesIds[$item->id] * ($item->purchase_line['purchase_price_inc_tax'] ?? 1);
            });
            // -------------------------------------------------- \\
            $this->saleEntriesPosService->returnedSellProducts(
                $sell_return,
                $sumPricesPurchases,
                $sell
            );
    ...
    public function getPdfContentsForGivenTransaction($business_id, $transaction_id)
    {
        ...
        //Get receipt details
        $receipt_details = $this->getReceiptDetails(
            $transaction_id,
            $transaction->location_id,
            $invoice_layout,
            $business_details,
    ...
    SUM(CASE 
        ...
        THEN COALESCE(discount_amount, 0) * total_before_tax / 100 
        ELSE COALESCE(discount_amount, 0) 
...
"),
    ...
    private function discountAmountPerItem($linesCount, $totalInvoiceDiscount)
    {
        return $totalInvoiceDiscount / $linesCount;
    }
    private function totalTaxAfterDiscount($line, $discountPerItem)
    {
        $lineTotal = $line['line_total_exc_tax_uf'] - $discountPerItem;
        $price = $this->calc_percentage($lineTotal, $line['tax_percent'], 0);
        return $price;
    }
}
