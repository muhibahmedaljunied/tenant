...
class AccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            $accounts = Account::leftjoin('account_transactions as AT', function ($join) {
                ...
                ->select([
                    ...
                    'is_closed',
                    DB::raw("SUM(CASE WHEN AT.type = 'credit' THEN amount ELSE -1 * amount END) AS balance"),
        ...
        return view('account.index')
            ->with(compact('not_linked_payments', 'account_types', 'menuItems'));
    }
        ...
        if (request()->ajax()) {
            $accounts = AccountTransaction::join(
                ...
                ->select([
                    ...
                    DB::raw("(
                        SELECT SUM(
                            IIF(AT.type = 'credit', AT.amount, -1 * AT.amount)
                        )
                        FROM account_transactions AS AT
                        WHERE AT.operation_date <= account_transactions.operation_date
                          AND AT.account_id = account_transactions.account_id
                          AND AT.deleted_at IS NULL
                          AND AT.id <= account_transactions.id
                    ) AS balance"),
                ...
                ->orderBy('account_transactions.operation_date', 'asc')->offset(0)
                ->limit(1000000);
            ...
            if (!empty($start_date) && !empty($end_date)) {
                $accounts->whereBetween(DB::raw('CONVERT(date,operation_date)'), [$start_date, $end_date]);
            }
    ...
    public function cashFlow(Request $request)
    {
            ...
            $accounts = AccountTransaction::join(
                ...
                ->select([
                    ...
                    DB::raw("(
                        ...
                        FROM account_transactions AS AT
                        JOIN accounts AS ac ON ac.id = AT.account_id
                        WHERE ac.business_id = $business_id
                          AND AT.operation_date <= account_transactions.operation_date
                          AND AT.deleted_at IS NULL
                    ) AS balance"),
                ...
                ->orderBy('account_transactions.operation_date', 'desc')->offset(0)
                ->limit(1000000);
            if (!empty(request()->input('type'))) {
        ...
        return view('account.cash_flow')
            ->with(compact('accounts', 'menuItems'));
    }
