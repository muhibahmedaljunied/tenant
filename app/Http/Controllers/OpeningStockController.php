...
use Modules\ChartOfAccounts\Factories\ProductBalance;
...
class OpeningStockController extends Controller
{
        ...
        if (!empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                ...
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['patch_number'] = $purchase_line->patch_number;
                }
            ...
            $enable_patch = request()->session()->get('business.enable_patch_number');
     ...
     * @return \Illuminate\Http\Response
     */
    public function save(Request $request, ProductBalance $productBalance)
    {
        if (!auth()->user()->can('product.opening_stock')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            $product = Product::where('business_id', $business_id)
                ->where('id', $product_id)
                ->with([
                    'variations'  => fn ($q) => $q->whereNull('parent_variation_id'),
                    'product_tax'
                    ])
                ->first();
            $settings = AcSetting::where('business_id', $business_id)->first();
                            ...
                            foreach ($rows as $key => $v) {
                                ...
                                if (isset($v['purchase_line_id'])) {
                                    ...
                                    if ($qty_remaining != 0) {
                                        ...
                                        $productBalance->setOpenStock(false)->update($product->id, $location_id, [
                                            'amount' => $purchase_price,
                                            'qty' => $qty_remaining,
                                            'description' => $product->name,
                                            'date' => $transaction_date,
                                            'opening_account' => $settings->stock_opening_account,
                                        ]);
                                    }
                                } else {
                                    if ($qty_remaining != 0) {
