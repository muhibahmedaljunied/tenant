    ...
    public function getProductStockAlert()
    {
        if (request()->ajax()) {
            ...
            $query = VariationLocationDetails::join(
                ...
                ->whereRaw('CAST(variation_location_details.qty_available AS FLOAT) <= p.alert_quantity'); // SQL Server-safe casting
            ...
            $products = $query->select(
                // DB::raw('TOP 100 PERCENT p.name as product'),
                'p.name as product',
                'p.type',
            ...
            return Datatables::of($products)->order(function ($query) {
                // DataTables will handle ordering automatically on the client side
                // You may define server-side sort here if needed with TOP/OFFSET
            })
