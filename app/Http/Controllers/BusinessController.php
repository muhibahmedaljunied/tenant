...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getRegister()
    {
        ...
        // Log::info("message_in_getRegister" . DB::connection()->getDatabaseName());
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            // ----------------------------muhib add this because in sqlserver you cannot insert if with you field nut null---------------
            $business_details['sharenumber'] = 0;
            $business_details['capital'] = 0;
            // ---------------
            //upload logo
            ...
            Log::info('error of register buseniss'.$e->getMessage());
            dd($e->getMessage());
            // throw $e;
            DB::rollBack();
    ...
    public function getBusinessSettings(Request $request)
    {
        ...
        $zatcaSetting = optional(Zatca::latest('id')->first());


        $menuItems = $request->menuItems;

        return view('business.settings', get_defined_vars(), compact('menuItems'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            if ($zatca && $request->zatca_phase == Zatca::STATUS_PHASE2 && ! $zatca->isZatcaConfigured()) {
                $business_details['zatca_phase'] = $request->input('zatca_phase');

                $this->handleZatcaUpdate($request->collect('zatca_settings'));
            }
            ...
            $business_details['enable_patch_number'] = ! empty($request->input('enable_patch_number')) && $request->input('enable_patch_number') == 1 ? 1 : 0;
            ...
            $business_id = request()->session()->get('user.business_id');
            $business = Business::whereId($business_id)->firstOrFail();
    ...
    public function updateZatcaSetting(Request $request)
    {
        return rescue(function () use ($request) {
            ...
            return back()->with('status', [
                'success' => 1,
                'msg' => __('zatca.msg.updated_success')
            ]);
    ...
    private function handleZatcaUpdate($zatcaValues)
    {
        return $this->zatcaService
            ->validateRequest($zatcaValues)
            ->updateBusinessSettings($zatcaValues)
            ->updateOnBoardingGateway();
    }
}
