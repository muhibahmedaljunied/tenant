...
class ReportController extends Controller
{
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        return view('report.profit_loss', compact('business_locations', 'menuItems'));
    }
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        return view('report.purchase_sell')
            ->with(compact('business_locations', 'menuItems'));
    }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        //Return the details in ajax call
        if ($request->ajax()) {
            // Step 1: Precalculate payments per transaction
            // Step 1: Prepare payment totals per transaction
            $payments = DB::table('transaction_payments')
                ->select([
                    'transaction_id',
                    DB::raw("SUM(CASE WHEN is_return = 1 THEN -1 * amount ELSE amount END) AS total_paid")
                ])
                ->groupBy('transaction_id');

            // Step 2: Base query with joins and filters
            $contacts = DB::table('contacts')
                ->join('transactions AS t', 'contacts.id', '=', 't.contact_id')
                ->leftJoin(DB::raw("({$payments->toSql()}) as tp"), 'tp.transaction_id', '=', 't.id')
            ...
            // Step 3: Grouping and Aggregated Selects
            $contacts->groupBy(
                'contacts.id',
                ...
                DB::raw("SUM(CASE WHEN t.type = 'purchase' THEN tp.total_paid ELSE 0 END) AS purchase_paid"),
                DB::raw("SUM(CASE WHEN t.type = 'sell' AND t.status = 'final' THEN tp.total_paid ELSE 0 END) AS invoice_received"),
                DB::raw("SUM(CASE WHEN t.type = 'sell_return' THEN tp.total_paid ELSE 0 END) AS sell_return_paid"),
                DB::raw("SUM(CASE WHEN t.type = 'purchase_return' THEN tp.total_paid ELSE 0 END) AS purchase_return_received"),
                DB::raw("SUM(CASE WHEN t.type = 'opening_balance' THEN tp.total_paid ELSE 0 END) AS opening_balance_paid"),
            ...
            // Step 3: Apply location filter if needed
            $permitted_locations = auth()->user()->permitted_locations();
            if ($permitted_locations !== 'all') {
                $contacts->whereIn('t.location_id', $permitted_locations);
            }

            // Step 4: Apply group filters from request
            if (!empty($request->input('customer_group_id'))) {
                $contacts->where('contacts.customer_group_id', $request->input('customer_group_id'));
            }
            if (!empty($request->input('contact_type'))) {
        ...
        return view('report.contact')
            ->with(compact('customer_group', 'types', 'menuItems'));
    }
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $datatable = Datatables::of($products)
                ...
                ->editColumn('default_purchase_price', function ($row) {
                    ...
                    if (auth()->user()->can('access_default_selling_price')) {
                        $html .= "<span class='display_currency' data-currency_symbol='true'>{$row->default_purchase_price}</span>";
                    }
                    return $html;

                    // return '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float) $row->default_purchase_price . '" data-unit="' . $row->default_purchase_price . '" data-currency_symbol=true > ' . (float) $row->default_purchase_price . '</span>' . ' ';
                })
                ...
                // ->editColumn('purchase_price_without_tax', function ($row) {
                //     $html = '';
                //     if (auth()->user()->can('access_default_selling_price')) {
                //         $html .= "<span class='display_currency' data-currency_symbol='true'>{$row->unit_price_without_tax}</span>";
                //     }
                //     return $html;
                // })
                ->editColumn('unit_price_without_tax', function ($row) {
                    $html = '';
                    if (auth()->user()->can('access_default_selling_price')) {
                        $html .= "<span class='display_currency' data-currency_symbol='true'>{$row->unit_price_without_tax}</span>";
                    }
                ...
                ->editColumn('stock_value_by_sale_price_without_tax', function ($row) {
                    $unit_selling_price = (float) $row->unit_price_without_tax;
                    $stock_price = (float) ($row->stock ? $row->stock : 0) * $unit_selling_price;

                    return "<span class='stock_value_by_sale_price_without_tax display_currency' data-orig-value='{$stock_price}' data-currency_symbol=true >{$stock_price}</span>";
                })
                ->addColumn('total_purchase_price_without_tax', function ($row) {
                    $total = (float) $row->default_purchase_price * $row->stock;
                    // $total = ($totalPurchasePrices >= 0 ? $totalPurchasePrices : $row->default_purchase_price);

                    return "<span class='total_purchase_price_without_tax display_currency' data-orig-value='{$total}' data-currency_symbol='true'>" . (float) $total . "</span>";
                })
            ...
            $raw_columns = [
                ...
                'total_transfered',
                'stock_value_by_sale_price_without_tax',
        ...
        if ($request->ajax()) {
            ...
            $product_details = $query->select(
                ...
                DB::raw("(
                    SELECT SUM(
                        CASE 
                            WHEN transactions.type = 'sell' 
                            THEN TSL.quantity - TSL.quantity_returned 
                            ELSE -1 * TPL.quantity 
                        END
                    )
                    FROM transactions
                ...
                DB::raw("(
                    ...
                    WHERE transactions.status = 'received'
                      AND transactions.type = 'stock_adjustment'
                      $location_filter
            ...
            return view('report.stock_details')
                ->with(compact('product_details', 'menuItems'));
        }
    }
    ...
    public function getTaxDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $datatable
                ...
                ->editColumn('total_after_tax', function ($row) {
    ...
    public function getTaxReport(Request $request)
    {
        ...
        return view('report.tax_report')
            ->with(compact('business_locations', 'taxes', 'tax_report_tabs', 'menuItems'));
    }
    public function getTaxSellPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return $this->transactionUtil->getTotalSellPurchaseWithTax($business_id, $start_date, $end_date, $location_id);
        }
    }
    ...
    public function getLessTrendingProducts(Request $request)
    {
        ...
        return view('report.less_trending_products')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'menuItems'));
    }
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        return view('report.expense_report')
            ->with(compact('categories', 'business_locations', 'expenses', 'menuItems'));
    }
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
        ...
        return view('report.sales_representative')
            ->with(compact('users', 'business_locations', 'menuItems'));
    }
    ...
    public function getStockExpiryReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $report = $query->select(
                ...
                ->having(DB::raw("SUM(COALESCE(quantity, 0) - COALESCE(quantity_sold, 0) - COALESCE(quantity_adjusted, 0) - COALESCE(quantity_returned, 0))"), '>', 0)
                ->groupBy(
    ...
    public function getStockExpiryReportEditModal(Request $request, $purchase_line_id)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('report.partials.stock_expiry_edit_modal')
                ->with(compact('purchase_line', 'menuItems'));
        }
    }
    ...
    public function getCustomerGroup(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Transaction::leftjoin('customer_groups AS CG', 'transactions.customer_group_id', '=', 'CG.id')
                ...
                ->where('transactions.status', 'final')
                ->groupBy('transactions.customer_group_id', 'CG.name')
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                ->leftJoin('variations as parent_variation', 'v.parent_variation_id', '=', 'parent_variation.id')
                ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                ->join('contacts as c', 't.contact_id', '=', 'c.id')
                ->join('products as p', 'pv.product_id', '=', 'p.id')
                ->leftjoin('units as variation_unit', 'v.unit_id', '=', 'variation_unit.id')
                ->leftjoin('units as u', 'p.unit_id', '=', 'u.id')
                ->where('t.business_id', $business_id)
                ...
                ->select(
                    ...
                    'variation_unit.short_name  as v_unit',
                    'variation_unit.id          as v_unit_id',
                    // 'variation_unit.allow_decimal as vu_unit_allow_decimal',

                    'parent_variation.equal_qty as variation_equal_qty',
                ...
                ->groupBy(
                    ...
                    'variation_unit.short_name',
                    'variation_unit.id',
                    'parent_variation.equal_qty',
                    // -----------------------------------------------
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_qty', function ($row) {
                    $unit = $row->v_unit ?? $row->unit;
                    return '<span data-is_quantity="true" class="display_currency purchase_qty" data-currency_symbol=false data-orig-value="' . (float) $row->purchase_qty . '" data-unit="' . $unit . '" >' . (float) $row->purchase_qty . '</span> ' . $unit;
                })
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query->orderby('transaction_id')
                ->offset(0)
                ->limit(1000000); // or any large number to fetch all rows
    ...
    public function getproductSellReturnReport(Request $request)
    {
        ...
        return view('report.product_sell_return_report')
            ->with(compact('business_locations', 'customers', 'menuItems'));
    }
    ...
    public function getproductSellReportWithPurchase(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query->orderby('transaction_id')->offset(0)
                ->limit(1000000); // or any large number to fetch all rows;
    ...
    public function getLotReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                DB::raw("(
                    COALESCE((
                        SELECT SUM(pls.quantity - pls.quantity_returned)
                        FROM purchase_lines AS pls
                        WHERE $location_filter
                          AND pls.variation_id = v.id
                ...
                COALESCE(
                    SUM(
                        CASE 
                            WHEN tspl.sell_line_id IS NULL THEN 0 
                            ELSE tspl.quantity - tspl.qty_returned 
                        END
                    ), 0
                ) AS total_sold
            "),
                ...
                COALESCE(
                    SUM(
                        CASE 
                            WHEN tspl.stock_adjustment_line_id IS NULL THEN 0 
                            ELSE tspl.quantity 
                        END
                    ), 0
                ) AS total_adjusted
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    DB::raw("
                    CASE 
                        WHEN transaction_payments.transaction_id IS NULL THEN (
                            SELECT TOP 1 c.name 
                            FROM transactions as ts
                            JOIN contacts as c ON ts.contact_id = c.id
                            WHERE ts.id = (
                                SELECT TOP 1 tps.transaction_id 
                        ...
                        ELSE (
                            SELECT 
                                COALESCE(c.supplier_business_name, '') + '<br>' + c.name
                            FROM transactions as ts
                            JOIN contacts as c ON ts.contact_id = c.id
                            WHERE ts.id = t.id
                        )
            ...
            if (! empty($start_date) && ! empty($end_date)) {
                $query->whereBetween(
                    DB::raw("CONVERT(DATE, paid_on) as paid_date"),
                    [$start_date, $end_date]
                );
            }
        ...
        return view('report.purchase_payment_report')
            ->with(compact('business_locations', 'suppliers', 'menuItems'));
    }
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    ...
                    ELSE (
                        SELECT TOP 1 COALESCE(c.supplier_business_name, '') + '<br>' + c.name
                        FROM transactions AS ts
                        JOIN contacts AS c ON ts.contact_id = c.id
                        WHERE ts.id = t.id
                    )
        ...
        return view('report.sell_payment_report')
            ->with(compact('business_locations', 'customers', 'payment_types', 'customer_groups', 'menuItems'));
    }
    ...
    public function getTableReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (! empty($start_date) && ! empty($end_date)) {
                $query->whereBetween(
                    DB::raw("CONVERT(DATE, transaction_date) AS transaction_date_only"),
                    [$start_date, $end_date]
                );
            }
    ...
    public function getServiceStaffReport(Request $request)
    {
        ...
        return view('report.service_staff_report')
            ->with(compact('business_locations', 'waiters', 'menuItems'));
    }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    't.id as transaction_id',
                    't.transaction_date as transaction_date',
                    DB::raw("FORMAT(t.transaction_date, 'yyyy-MM-dd') AS formated_date"),
                ...
                ->groupBy(DB::raw("FORMAT(t.transaction_date, 'yyyy-MM-dd')"));
     ...
     * @return \Illuminate\Http\Response
     */
    public function productStockDetails(Request $request)
    {
        ...
        return view('report.product_stock_details')
            ->with(compact('stock_details', 'business_locations', 'location', 'menuItems'));
    }
    ...
    public function getProfit($by = null)
    {
        ...
        // ----------------------------------------------------------------------------------------------
        // $subquery = DB::table('transaction_sell_lines as tsl')
        ...
        //             SELECT SUM(
        //               (tspl2.quantity - tspl2.qty_returned) * 
        //               (tsl2.unit_price_inc_tax - pl2.purchase_price_inc_tax)
        //             )
        //             FROM transaction_sell_lines AS tsl2
        //             JOIN transaction_sell_lines_purchase_lines AS tspl2 ON tsl2.id = tspl2.sell_line_id
        //             JOIN purchase_lines AS pl2 ON tspl2.purchase_line_id = pl2.id
        //             WHERE tsl2.parent_sell_line_id = tsl.id
        //           )
        //           WHEN p.enable_stock = 0 THEN 
        //             (tsl.quantity - tsl.quantity_returned) * tsl.unit_price_inc_tax
        //           ELSE 
        //             (tspl.quantity - tspl.qty_returned) * 
        //             (tsl.unit_price_inc_tax - pl.purchase_price_inc_tax)
        //         END AS gross_profit
        //     ")
        //     ])
        ...
        //     ->select(DB::raw('SUM(gross_profit) as gross_profit'));
        ...
        //           JOIN transaction_sell_lines_purchase_lines AS tspl2 ON tsl2.id = tspl2.sell_line_id
        //           JOIN purchase_lines AS pl2 ON tspl2.purchase_line_id = pl2.id
        //           WHERE tsl2.parent_sell_line_id = transaction_sell_lines.id
        //         )
        //         WHEN P.enable_stock = 0 THEN 
        //           (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax
        //         ELSE 
        //           (TSPL.quantity - TSPL.qty_returned) * 
        //           (transaction_sell_lines.unit_price_inc_tax - PL.purchase_price_inc_tax)
        //       END
        //     ) AS gross_profit
        //   ")
        ...
        //         END AS product
        //     "),
        //         DB::raw('tsl.variation_id'),
        //         DB::raw('sale.transaction_date'),
        //         DB::raw('sale.location_id'),
        //         DB::raw('sale.contact_id'),
        //         'p.category_id',
        //         'p.brand_id',
        //         'sale.invoice_no',
        ...
        //     ->where('sale.business_id', $business_id)
        //     ->where('tsl.children_type', '!=', 'combo')
        //     ->whereBetween(DB::raw('CAST(sale.transaction_date AS date)'), ['2025-01-01', '2025-12-31']);
        ...
        //     $start = request()->start_date;
        //     $end = request()->end_date;
        //     $query->whereBetween(DB::raw('CAST(derived.transaction_date AS date)'), [$start, $end]);
        // }

        // if ($by === 'product') {
        //     $query->join('variations as V', 'derived.variation_id', '=', 'V.id')
        //         ->leftJoin('product_variations as PV', 'PV.id', '=', 'V.product_variation_id')
        ...
        //           END AS product
        //       "))
        //         ->groupBy('V.id', 'V.sub_sku', 'P.type', 'p.name', 'pv.name', 'v.name', 'p.sku');
        // }

        // if ($by === 'category') {
        //     $query->join('products as P', 'derived.variation_id', '=', 'P.id')
        //         ->leftJoin('categories as C', 'C.id', '=', 'P.category_id')
        //         ->addSelect('C.name as category')
        //         ->groupBy('C.id', 'product', 'P.type', 'C.name', 'p.sku');
        // }
        ...
        //         ->groupBy('B.id', 'product', 'B.name', 'p.sku');
        // }

        // if ($by === 'location') {
        //     $query->join('business_locations as L', 'derived.location_id', '=', 'L.id')
        //         ->addSelect('L.name as location')
        //         ->groupBy('L.id', 'product', 'L.name');
        // }
        ...
        //         'derived.discount_type',
        //         'derived.discount_amount',
        //         'derived.total_before_tax'
        //     )
        //         ->groupBy(
        //             'derived.invoice_no',
        //             'derived.transaction_id',
        ...
        // if ($by === 'date') {
        //     $query->addSelect(DB::raw('CAST(derived.transaction_date AS DATE) as date'))
        //         ->groupBy(DB::raw('CAST(derived.transaction_date AS DATE)'));
        // }

        // if ($by === 'day') {
        //     $results = $query->addSelect(DB::raw('DATENAME(WEEKDAY, derived.transaction_date) as day'))
        //         ->groupBy(DB::raw('DATEPART(WEEKDAY, derived.transaction_date)'), 'derived.transaction_date')
        //         ->get();
        ...
        // if ($by === 'customer') {
        //     $query->join('contacts as CU', 'derived.contact_id', '=', 'CU.id')
        //         ->addSelect('CU.name as customer', 'CU.supplier_business_name')
        //         ->groupBy('derived.contact_id', 'CU.name', 'CU.supplier_business_name');
        // }

        // $query->groupBy('P.type', 'p.name', 'pv.name', 'v.name', 'p.sku');
        ...
        // ✅ Gross profit calculation (SQL Server safe)
        $query->select(DB::raw("
    SUM(
        CASE 
            WHEN TSPL.id IS NULL AND P.type = 'combo' THEN 0
            WHEN P.enable_stock = 0 THEN 
                (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax
            ELSE 
...
"));
        ...
        if ($by === 'product') {
            $query->join('variations as V', 'transaction_sell_lines.variation_id', '=', 'V.id')
                ...
                ->addSelect(DB::raw("
              CASE 
                  WHEN P.type = 'variable' THEN CONCAT(P.name, ' - ', PV.name, ' - ', V.name, ' (', V.sub_sku, ')')
                  ELSE CONCAT(P.name, ' (', P.sku, ')')
              END as product
          "))
                ->groupBy('V.id', 'P.name', 'PV.name', 'V.name', 'V.sub_sku', 'P.type', 'P.sku');
        }
        ...
        if ($by === 'date') {
            $query->addSelect(DB::raw('CAST(sale.transaction_date AS DATE) as transaction_date'))
                ->groupBy(DB::raw('CAST(sale.transaction_date AS DATE)'), 'sale.transaction_date');
        }

        if ($by === 'day') {
            $results = $query->addSelect(DB::raw("DATENAME(WEEKDAY, sale.transaction_date) as day"))
                ->groupBy(DB::raw("DATEPART(WEEKDAY, sale.transaction_date)"), DB::raw("DATENAME(WEEKDAY, sale.transaction_date)"))
                ->get();
        ...
        if ($by === 'customer') {
            $query->join('contacts as CU', 'sale.contact_id', '=', 'CU.id')
                ->addSelect('CU.name as customer', 'CU.supplier_business_name')
                ->groupBy('sale.contact_id', 'CU.name', 'CU.supplier_business_name');
        }
    ...
    public function itemsReport(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            if (true) {
                $query->selectRaw('SUM(transaction_sell_lines_purchase_lines.quantity) as quantity')
                    ->selectRaw('SUM(SL.unit_price_inc_tax) as total_selling_price')
                    ->groupBy(
                        'p.id',
                        'customers.id',
                        \DB::raw('MONTH(stock_adjustment.transaction_date)'),
                        ...
                        'customers.name',
                        'customers.supplier_business_name',
                        'SL.unit_price_inc_tax',
                        'SAL.unit_price',
                        'transaction_sell_lines_purchase_lines.stock_adjustment_line_id',
                        'transaction_sell_lines_purchase_lines.sell_line_id',
                        'transaction_sell_lines_purchase_lines.purchase_line_id',
            ...
            return Datatables::of($query)
                ...
                ->editColumn('quantity', function ($row) {
                    $html = "<span data-is_quantity='true' class='display_currency quantity' data-currency_symbol='false' data-orig-value='{$row->quantity}' data-unit='{$row->unit}'>{$row->quantity}</span><span class='unit'>{$row->unit}</span>";
                ...
                ->editColumn('sell_date', function ($row) {
                    ...
                    $stockAdjustmentDate = Carbon::parse($sell_date);

                    $stockAdjustmentDate->locale('ar');

                    $currentYear = Carbon::now()->year;
                    $formattedDate = $stockAdjustmentDate->isoFormat('MMMM');

                    if ($stockAdjustmentDate->year != $currentYear) {
                        $formattedDate .= ' - ' . $stockAdjustmentDate->year;
                    }
                ...
                ->addColumn('subtotal', function ($row) {
                    $selling_price = ! empty($row->sell_line_id) ? $row->selling_price : $row->stock_adjustment_price;
                    //  $subtotal = $row->total_selling_price;
    ...
    public function sell_purchase_lines_report()
    {
        ...
        $business_locations = BusinessLocation::forDropdown($business_id);
        $menuItems = $request->menuItems;
        return view('report.sell_purchase_lines_report')->with(compact(
            'suppliers',
            'customers',
            'business_locations',
            'products',
    ...
    public function compare_locations(Request $request)
    {
        ...
        return view('report.stock-compares')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'show_manufacturing_data', 'menuItems'));
    }
    /**
        ...
        if (request()->ajax()) {
            ...
            $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    'discount_type',
                    'tax_amount',
                    DB::raw('FORMAT(transaction_date, "%Y/%m") as purchase_year_month'),
                    DB::raw('FORMAT(transaction_date, "%d") as purchase_day')
                )
                ->groupBy(
                    'transactions.id',
            ...
            if (! empty(request()->input('payment_status')) && request()->input('payment_status') != 'overdue') {
                ...
                $purchases->whereIn('transactions.payment_status', ['due', 'partial'])
                    ...
                    ->whereRaw("
                    CASE 
                      WHEN transactions.pay_term_type = 'days' 
                        THEN DATEADD(DAY, transactions.pay_term_number, transactions.transaction_date)
                      ELSE DATEADD(MONTH, transactions.pay_term_number, transactions.transaction_date)
    ...
    public function activityLog(Request $request)
    {
        ...
        $menuItems = $request->menuItems;
        return view('report.activity_log')->with(compact(
            'users',
            'transaction_types',
            'menuItems'
        ));
