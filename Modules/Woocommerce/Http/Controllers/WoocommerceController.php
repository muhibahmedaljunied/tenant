...
class WoocommerceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('woocommerce::woocommerce.index')
            ->with(compact('tax_rates', 'woocommerce_tax_rates', 'alerts', 'menuItems'));
    }
     ...
     * @return Response
     */
    public function viewSyncLog(Request $request)
    {
        $business_id = request()->session()->get('business.id');
        if (!(auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        $menuItems = $request->menuItems;
        return view('woocommerce::woocommerce.sync_log', compact('menuItems'));
    }
