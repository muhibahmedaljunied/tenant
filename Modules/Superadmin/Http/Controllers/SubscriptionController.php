...
class SubscriptionController extends BaseController
{
        ...
        try {
            ...
            if ($package->price == 0) {
                ...
                if (empty($form_register)) {
                    ...
                    return redirect()
                        // ->route('\Modules\Superadmin\Http\Controllers\SubscriptionController@index')
    ...
    protected function pay_paypal($business_id, $business_name, $package, $request)
    {
        ...
        // $data['return_url'] = route('\Modules\Superadmin\Http\Controllers\SubscriptionController@confirm', [$package->id]);
        // $data['cancel_url'] = route('\Modules\Superadmin\Http\Controllers\SubscriptionController@pay', [$package->id]);
        $data['return_url'] = route('subscription-confirm', [$package->id]);
        $data['cancel_url'] = route('subscription-pay', [$package->id]);
    ...
    public function paypalExpressCheckout(Request $request, $package_id)
    {
        ...
        // $data['invoice_description'] = "Order #{$data['invoice_id']} Invoice";
        // $data['return_url'] = route('\Modules\Superadmin\Http\Controllers\SubscriptionController@confirm', [$package_id]) . '?gateway=paypal';
        // $data['cancel_url'] = route('\Modules\Superadmin\Http\Controllers\SubscriptionController@pay', [$package_id]);

        $data['return_url'] = route('subscription-confirm', [$package_id]) . '?gateway=paypal';
        $data['cancel_url'] = route('subscription-pay', [$package_id]);
        $data['total'] = (float)$package->price;
    ...
    public function allSubscriptions()
    {
        ...
        return Datatables::of($subscriptions)
            ...
            ->addColumn('action', function ($row) {
                ...
                return '<button type="button" class="btn btn-primary btn-xs btn-modal" data-container=".view_modal" data-href="' . route("subscription-show", $row->id) . '" ><i class="fa fa-eye" aria-hidden="true"></i> ' . __("messages.view") . '</button>';
