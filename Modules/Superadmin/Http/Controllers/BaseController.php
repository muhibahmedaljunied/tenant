...
class BaseController extends Controller
{
    ...
    protected function _get_package_dates($business_id, $package)
    {
        ...
        // dd('cccc',$package->interval_count,'ffffff',$package->interval,'ggggggg',$output['start']);
        //Calculate end date

        // ------------------muhib add (int)----------
        if ($package->interval == 'days') {
            $output['end'] = $start_date->addDays((int)$package->interval_count)->toDateString();
        } elseif ($package->interval == 'months') {
            $output['end'] = $start_date->addMonths((int)$package->interval_count)->toDateString();
        } elseif ($package->interval == 'years') {
            $output['end'] = $start_date->addYears((int)$package->interval_count)->toDateString();
        }

        $output['trial'] = $start_date->addDays((int)$package->trial_days);
