<tr>
	...
	<td>
		<div class="input-group">
			<input type="text"
				name="ingredients[{{ $row_index }}][waste_percent]"
				value="{{ !empty($ingredient->waste_percent) ? @num_format($ingredient->waste_percent) : 0 }}"
	...
	<td>
		<div class="{{ empty($ingredient->sub_units) ? 'input-group' : 'input_inline' }}">
			<input type="text"
				name="ingredients[{{ $row_index }}][quantity]"
				value="{{ !empty($ingredient->quantity) ? @num_format($ingredient->quantity) : 1 }}"
