...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_mfg_enabled && (auth()->user()->can('manufacturing.access_recipe') || auth()->user()->can('manufacturing.access_production'))) {
            $manufacturingMenu = [
                'title' => __('manufacturing::lang.manufacturing'),
                'icon' => 'fa fas fa-industry',
                'order' => 23,
                'sub' => []
            ];
            ...
            if (auth()->user()->can('manufacturing.access_recipe')) {
                $manufacturingMenu['sub'][] = [
                    'title' => __('manufacturing::lang.recipe'),
                    'url' => action('\Modules\Manufacturing\Http\Controllers\RecipeController@index'),
                    'icon' => 'fa fas fa-hamburger',
                    'active' => request()->segment(1) == 'manufacturing' && in_array(request()->segment(2), ['recipe', 'add-ingredient'])
                ];
            }
            ...
            if (auth()->user()->can('manufacturing.access_production')) {
                $manufacturingMenu['sub'][] = [
                    ...
                    'icon' => 'fa fas fa-cogs',
                    'active' => request()->segment(2) == 'production' && empty(request()->segment(3))
                ];
                $manufacturingMenu['sub'][] = [
                    'title' => __('manufacturing::lang.add_production'),
                    'url' => action('\Modules\Manufacturing\Http\Controllers\ProductionController@create'),
                    'icon' => 'fa fas fa-plus',
                    'active' => request()->segment(2) == 'production' && request()->segment(3) == 'create'
                ];
                $manufacturingMenu['sub'][] = [
                    'title' => __('messages.settings'),
                    'url' => action('\Modules\Manufacturing\Http\Controllers\SettingsController@index'),
                    'icon' => 'fa fas fa-wrench',
                    'active' => request()->segment(1) == 'manufacturing' && request()->segment(2) == 'settings'
                ];
                $manufacturingMenu['sub'][] = [
                    'title' => __('manufacturing::lang.manufacturing_report'),
                    'url' => action('\Modules\Manufacturing\Http\Controllers\ProductionController@getManufacturingReport'),
                    'icon' => 'fa fas fa-chart-line',
                    'active' => request()->segment(2) == 'report'
                ];
            }

            $menuItems[] = $manufacturingMenu;
        }
