            ...
            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    ...
                    if (auth()->user()->can("job_sheet.view_assigned") || auth()->user()->can("job_sheet.view_all") || auth()->user()->can("job_sheet.create")) {
                        $html .= '<li>
                                <a href="' . action('\Modules\Repair\Http\Controllers\GuaranteeController@show', [$row->id]) . '" class="cursor-pointer">
                                    <i class="fa fa-eye"></i>
                                    ' . __("messages.view") . '
                                </a>
                                </li>
                    ...
                    if (auth()->user()->can("job_sheet.create") || auth()->user()->can("job_sheet.edit")) {
                        $html .= '<li>
                                    <a data-href="' . action('\Modules\Repair\Http\Controllers\GuaranteeController@editStatus', [$row->id]) . '" class="cursor-pointer edit_job_sheet_status">
