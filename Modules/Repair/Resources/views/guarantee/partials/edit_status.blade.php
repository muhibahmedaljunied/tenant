<div class="modal-dialog" role="document">
	<div class="modal-content">
		<form action="{{ action('\\Modules\\Repair\\Http\\Controllers\\GuaranteeController@updateStatus', [$guarantee->id]) }}" method="POST" id="update_status_form">
			@csrf
			@method('PUT')
						...
						<div class="form-group">
							...
							<select name="status_id" id="status_id_modal" class="form-control select2" required style="width:100%">
								...
								@foreach($status_dropdown['statuses'] as $id => $status)
									<option value="{{ $id }}" @if($guarantee->status_id == $id) selected @endif>{{ $status }}</option>
								@endforeach
							</select>
