...
<div class="login-form col-md-12 col-xs-12 right-col-content">
    ...
    {{-- <form method="POST" action="{{action('\\Modules\\Repair\\Http\\Controllers\\CustomerRepairStatusController@postRepairStatus')}}" id="check_repair_status"> --}}
        <form method="POST" action="{{route('CustomerRepairStatus-postRepairStatus')}}" id="check_repair_status">

        @csrf
        <div class="form-group">
            <div class="multi-input">
                <select name="search_type" class="form-control width-60 pull-left">
                    <option value="job_sheet_no">{{ __('repair::lang.job_sheet_no') }}</option>
                    <option value="invoice_no">{{ __('sale.invoice_no') }}</option>
                </select>
                <input type="text" name="search_number" class="form-control width-40 pull-left" required placeholder="{{ __('repair::lang.job_sheet_or_invoice_no') }}">
            </div>
        </div><br><br>
        <div class="form-group">
