<div class="modal-dialog modal-lg" role="document">
    <form action="{{ action('\\Modules\\Project\\Http\\Controllers\\TaskController@update', $project_task->id) }}" id="project_task_form" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-12">
                       <div class="form-group">
                            ...
                            <input type="text" name="subject" id="subject" class="form-control" required value="{{ $project_task->subject }}">
                       </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                       <div class="form-group">
                            ...
                            <input type="text" name="start_date" id="start_date" class="form-control datepicker" readonly value="{{ !empty($project_task->start_date) ? @format_date($project_task->start_date) : '' }}">
                       </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="due_date">{{ __('project::lang.due_date') }}:</label>
                            <input type="text" name="due_date" id="due_date" class="form-control datepicker" readonly value="{{ !empty($project_task->due_date) ? @format_date($project_task->due_date) : '' }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="priority" id="priority" class="form-control select2" required style="width: 100%;">
                                ...
                                @foreach($priorities as $id => $name)
                                    <option value="{{ $id }}" {{ $project_task->priority == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id[]" id="user_id" class="form-control select2" multiple required style="width: 100%;">
                                @foreach($project_members as $id => $name)
                                    <option value="{{ $id }}" {{ collect($project_task->members->pluck('id'))->contains($id) ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
