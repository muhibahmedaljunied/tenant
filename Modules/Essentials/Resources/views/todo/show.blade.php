@if(count($todo) > 0)
	...
	<li class="todo_li">
		...
		<a href="{{route('toDo-show', $do->id)}}"><span class="text task_name" style="text-decoration:line-through;">
			{{ $do->task }}
		</span></a>
		...
		<a href="{{route('toDo-show', $do->id)}}"><span class="text task_name"> {{ $do->task }}</span></a>
		@endif
		<br>
		...
		<div class="tools">
			...
			<i class="fa fa-edit text-primary pull-right btn-modal cursor-pointer action_btn"  data-container="#task_modal" data-href="{{route('toDo-edit', $do->id)}}"></i>
		</div>
	</li>
	@endforeach
@else
