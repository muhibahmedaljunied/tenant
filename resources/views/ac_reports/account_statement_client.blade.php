@extends('layouts.app')
@section('title', __('chart_of_accounts.account_statement_client'))
...
@section('content')
    ...
    <section class="content-header">
        <h1>@lang('chart_of_accounts.account_statement_client')
        ...
        <div class="row">
            <div class="col-md-12">
                @component('components.filters', ['title' => __('report.filters')])
                    <form action="{{ route('ac_reports_account-statement-report_client') }}" method="GET">
                ...
                <div class="treeview">
                    <div class="inner-block-reports col-sm-12 col-xs-12 clearfix">
                        ...
                        @php
                            ...
                            $accountDaysDetails = \App\Models\AcJournalEntryDetail::whereIn(
                                'account_number',
                                $accountsMaster->pluck('account_number')->toArray(),
                            )
                                ...
                                ->get();
                            $accountAmountDetails = \App\Models\AcJournalEntryDetail::whereIn(
                                'account_number',
                                $accountsMaster->pluck('account_number')->toArray(),
                            )
                        ...
                        @foreach ($accounts_range as $key => $value)
                            ...
                            @php
                                ...
                                $positive = $accountAmountDetails
                                    ->where('account_number', $master_details->account_number)
                                    ->sum(function ($value) {
                                        if ($value['amount'] > 0) {
                                            return $value['amount'];
                                        }
                            ...
                            <div class="account_parent_content">
                                ...
                                <div class="content">
                                                        ...
                                                        @foreach ($journal_entry->sortBy('ac_journal_entry.entry_date') as $entry)
                                                            <tr>
                                                                ...
                                                                @php
                                                                    ...
                                                                    if (
                                                                        $entry->account_type->account_type == 'debtor'
                                                                    ) {
                                                                        $debtor = $positive;
                                                                        $creditor = $negative * -1;
                                                                    } else {
                                    ...
                                    <hr>
                                    @if (!empty($accountDaysDetails))
                                        @php
                                            $daysDetails = $accountDaysDetails->where(
                                                'account_number',
                                                $master_details->account_number,
                                            );

                                            $newDaysDetails = collect();
                                            if (count($daysDetails) > 0) {
                                                $daysDetails = $daysDetails->each(function ($item) use (
                                                    $newDaysDetails
                                                ) {
                                                    if ($item->ac_journal_entry->journal_entry_debt_ages->count()) {
                                                        return $item->ac_journal_entry->journal_entry_debt_ages->each(
                                                            function ($content) use ($newDaysDetails) {
                                                                return $newDaysDetails->push([
                                                                    'amount' => $content->amount,
                                                                    'entry_date' => $content->debtages_date,
                                                                ]);
                                                            },
                                                        );
                                                    }
                                                    return $newDaysDetails->push([
                                                        'amount' => $item->amount,
                                                        'entry_date' => $item->ac_journal_entry->entry_date,
                                                    ]);
                                                });
                                            }
                                        @endphp
