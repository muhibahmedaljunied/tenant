...
<section class="content">
  ...
  <form action="{{ action('PurchaseController@update', [$purchase->id]) }}" method="POST" id="add_purchase_form" enctype="multipart/form-data">
    @csrf
    @method('PUT')
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <select name="contact_id" class="form-control" id="supplier_id" required>
                    <option value="{{ $purchase->contact_id }}">{{ $purchase->contact->name }}</option>
                  </select>
            ...
            <div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="transaction_date" id="transaction_date" value="{{ @format_datetime($purchase->transaction_date) }}" class="form-control" readonly required>
                </div>
              </div>
            </div>
              ...
              <div class="form-group">
                ...
                <select name="status" class="form-control select2" required>
                  ...
                  @foreach($orderStatuses as $key => $value)
                    <option value="{{ $key }}" @if($purchase->status == $key) selected @endif>{{ $value }}</option>
                  @endforeach
                </select>
              </div>
            </div>
              ...
              <div class="form-group">
                ...
                <select name="location_id" class="form-control select2" id="location_id" disabled>
                  ...
                  @foreach($business_locations as $key => $value)
                    <option value="{{ $key }}" @if($purchase->location_id == $key) selected @endif>{{ $value }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="col-sm-3 @if(!$currency_details->purchase_in_diff_currency) hide @endif">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="number" name="exchange_rate" id="exchange_rate" value="{{ $purchase->exchange_rate }}" class="form-control" required step="0.001">
                </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                  <div class="multi-input">
                    ...
                    <br/>
                    <input type="number" name="pay_term_number" id="pay_term_number" value="{{ $purchase->pay_term_number }}" class="form-control width-40 pull-left" placeholder="{{ __('contact.pay_term') }}">
                    <select name="pay_term_type" class="form-control width-60 pull-left" id="pay_term_type">
                      <option value="">{{ __('messages.please_select') }}</option>
                      <option value="months" @if($purchase->pay_term_type == 'months') selected @endif>{{ __('lang_v1.months') }}</option>
                      <option value="days" @if($purchase->pay_term_type == 'days') selected @endif>{{ __('lang_v1.days') }}</option>
                    </select>
                  </div>
              </div>
          </div>
            ...
            <div class="col-sm-12">
                <table class="table">
                    ...
                    <td class="col-md-3">
                      <div class="form-group">
                        ...
                        <select name="discount_type" class="form-control select2" id="discount_type">
                          ...
                          <option value="fixed" @if($purchase->discount_type == 'fixed') selected @endif>{{ __( 'lang_v1.fixed' ) }}</option>
                          <option value="percentage" @if($purchase->discount_type == 'percentage') selected @endif>{{ __( 'lang_v1.percentage' ) }}</option>
                        </select>
                      </div>
                    </td>
                    <td class="col-md-3">
                      <div class="form-group">
                        ...
                        <input type="text" name="discount_amount" id="discount_amount"
                          value="@if($purchase->discount_type == 'fixed'){{ number_format($purchase->discount_amount/$purchase->exchange_rate, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}@else{{ number_format($purchase->discount_amount, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}@endif"
                  ...
                  <tr class="hide">
                    <td>
                      <div class="form-group">
                        ...
                        <input type="hidden" name="tax_amount" id="tax_amount" value="{{ $purchase->tax_amount }}">
                      </div>
                    </td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                  ...
                  <tr>
                    <td>
                      <div class="form-group">
                        ...
                        <input type="text" name="shipping_details" id="shipping_details" value="{{ $purchase->shipping_details }}" class="form-control">
                      </div>
                    </td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>
                      <div class="form-group">
                        ...
                        <input type="text" name="shipping_charges" id="shipping_charges" value="{{ number_format($purchase->shipping_charges/$purchase->exchange_rate, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}" class="form-control input_number">
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>&nbsp;</td>
                    <td>
                      <input type="hidden" name="final_total" id="grand_total_hidden" value="{{ $purchase->final_total }}">
    ...
    <div class="row">
        <div class="col-sm-12">
          <button type="submit" id="submit_purchase_form" class="btn btn-primary pull-right btn-flat">@lang('messages.update')</button>
        </div>
    </div>
  </form>
</section>
