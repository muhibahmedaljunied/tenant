        ...
        <div class="col-sm-12 col-xs-12">
            <div class="table-responsive">
                <table class="table bg-gray">
                    ...
                    @foreach ($purchase->purchase_lines as $purchase_line)
                        <tr>
                            ...
                            <td><span class="display_currency" data-is_quantity="true"
                                    data-currency_symbol="false">{{ $purchase_line->main_qty }}</span>
                                @if (!empty($purchase_line->sub_unit))
                                    {{ $purchase_line->sub_unit->short_name }}
                                @elseif($purchase_line->related_variations)
                                    {{ $purchase_line->related_variations->unit->short_name }}
                                @else
                            ...
                            <td class="no-print text-right"><span class="display_currency"
                                    data-currency_symbol="true">{{ ($purchase_line->main_qty_purchased ?? $purchase_line->main_qty) * $purchase_line->purchase_price }}</span>
                            </td>
                            <td class="text-right"><span class="display_currency"
                                    data-currency_symbol="true">{{ $purchase_line->item_tax }} </span> <br /><small>
                    ...
                    @endif
                    @if (session('business.enable_patch_number'))
                        <td>{{ $purchase_line->patch_number }}</td>
                    @endif
                    ...
                    @endif
                    <td class="text-right"><span class="display_currency"
                            data-currency_symbol="true">{{ $purchase_line->purchase_price_inc_tax * ($purchase_line->main_qty_purchased ?? $purchase_line->main_qty) }}</span>
                    </td>
                    </tr>
                    @php
                        $total_before_tax += ($purchase_line->main_qty_purchased ?? $purchase_line->main_qty) * $purchase_line->purchase_price;
                    @endphp
                    @endforeach
                </table>
            </div>
