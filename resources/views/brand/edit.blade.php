...
<form action="{{ action('BrandController@update', [$brand->id]) }}" method="POST" id="brand_edit_form">
  ...
  <input type="text" name="name" value="{{ $brand->name }}" class="form-control" required placeholder="{{ __('brand.brand_name') }}">
</div>

<div class="form-group">
  <label for="description">{{ __('brand.short_description') }}:</label>
  <input type="text" name="description" value="{{ $brand->description }}" class="form-control" placeholder="{{ __('brand.short_description') }}">
</div>
