    ...
    <table class="table table-bordered add-product-price-table table-condensed" id="product_variation_form_part">
        ...
        @foreach( $product_variations as $product_variation)
          ...
          <tr class="variation_row">
            <td>
              <input type="text" name="product_variation[{{ $loop->index }}][name]" value="{{ $product_variation->name }}" class="form-control input-sm variation_name" required>
              <input type="hidden" class="row_index" value="{{  $loop->index }}">
              <input type="hidden" name="product_variation[{{ $loop->index }}][product_variation_id]" value="{{ $product_variation->id }}">
            </td>
            <td>
                <table class="table table-condensed table-bordered blue-header variation_value_table">
                    ...
                    <tr>
                        <td>
                          <input type="text" name="product_variation[{{ $loop->parent->index }}][variations][{{ $loop->index }}][value]" value="{{ $variation->name }}" class="form-control input-sm variation_value_name" required>
                          <input type="hidden" name="product_variation[{{ $loop->parent->index }}][variations][{{ $loop->index }}][variation_id]" value="{{ $variation->id }}">
                        </td>
                        <td class="drag-select">
                          <input type="number" name="product_variation[{{ $loop->parent->index }}][variations][{{ $loop->index }}][default_purchase_price]" value="{{ $variation->default_purchase_price }}" class="form-control input-sm dpp" min="0">
                        </td>
                        <td class="drag-select">
                          <input type="number" name="product_variation[{{ $loop->parent->index }}][variations][{{ $loop->index }}][default_sell_price]" value="{{ $variation->default_sell_price }}" class="form-control input-sm variable_dsp" min="0" placeholder="{{ __('product.exc_of_tax') }}">
                          <input type="number" name="product_variation[{{ $loop->parent->index }}][variations][{{ $loop->index }}][sell_price_inc_tax]" value="{{ $variation->sell_price_inc_tax }}" class="form-control input-sm variable_dsp_inc_tax" min="0" placeholder="{{ __('product.inc_of_tax') }}">
                        </td>
                        <td>
