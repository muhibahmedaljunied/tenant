									...
									<table class="table table-condensed table-bordered  text-center table-striped">
										...
										@foreach($product->variations as $variation)
											<tr>
												...
												@endif
												<td>
													<input type="text" name="group_prices[{{$price_group->id}}][{{$variation->id}}]" value="{{ !empty($variation_prices[$variation->id][$price_group->id]) ? @num_format($variation_prices[$variation->id][$price_group->id]) : 0 }}" class="form-control input_number input-sm">
												</td>
											</tr>
										@endforeach
										</tbody>
