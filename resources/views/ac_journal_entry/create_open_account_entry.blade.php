@extends('layouts.app')
@section('title', __('chart_of_accounts.add_new_opening_journal_entry'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('chart_of_accounts.add_new_opening_journal_entry')</h1>
    <!-- <ol class="breadcrumb">
...
<section class="content">
    ...
    @endphp
    <form action="{{ action('AcJournalEntryController@storeOpenAccountEntry') }}" method="POST" id="journal_entry_add_form" class="journal_entry_form{{ $form_class }}" enctype="multipart/form-data">
        @csrf
            ...
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="entry_date" id="entry_date" class="form-control" required value="{{ \Carbon\Carbon::parse('now')->format('d/m/Y') }}">
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-condensed table-bordered table-striped table-responsive" id="journal_entry_table">
                    ...
                    <tbody class="journal_entry_rows" id="journal_entry_rows">
                        <tr class="journal_entry_row">
                            ...
                            <td>
                                <select name="journal_entries[0][cost_cen_field_id]" class="form-control select2 cost_cen_field_id" style="width: 100%;">
                                    <option value="">{{ __('Select Additional Cost Center') }}</option>
                                    @foreach($ac_cost_cen_field_adds as $key => $label)
                                    <option value="{{ $key }}">{{ $label }}</option>
                                    @endforeach
                                </select>
                            </td>
                ...
                <table class="table table-condensed table-bordered table-striped table-responsive">
                    <tbody>
                        ...
                        <tr style="background-color: greenyellow">
                            <td class="text-center"><span>المجموع :</span> <span id="total_debtor">0</span>
                                <input type="hidden" class="total_debtor" name="total_debtor" value="0">
                            </td>
                            <td> <span>المجموع :</span> <span id="total_creditor"> 0</span>
                                <input type="hidden" class="total_creditor" name="total_creditor" value="0">
                            </td>
                        ...
                        <tr>
                            ...
                            <td colspan="4" id="unbalanced_entry" style="color: red;display: none;">القيد غير متوازن
                                بمقدار <span id="unbalanced_entry_value_dif"></span>
                            </td>
