@extends('layouts.app')
@section('title', __('chart_of_accounts.edit_journal_entry'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('chart_of_accounts.edit_journal_entry')</h1>
    <!-- <ol class="breadcrumb">
...
<section class="content">
    <form action="{{ action('AcJournalEntryController@update', [$ac_journal_entry_record->id]) }}"
        method="POST"
        id="journal_entry_edit_form"
        class="journal_entry_form{{ empty($duplicate_product) ? 'create' : '' }}"
        ...
        <div class="row">
            ...
            <div class="col-sm-8">
                <div class="form-group">
                    ...
                    <input type="text" name="entry_desc" class="form-control" value="{{ $ac_journal_entry_record->entry_desc }}">
                </div>
            </div>
                ...
                <table class="table table-condensed table-bordered table-striped table-responsive" id="journal_entry_table">
                    ...
                    <tbody class="journal_entry_rows" id="journal_entry_rows">
                        ...
                        <tr class="journal_entry_row">
                            ...
                            <td>
                                <input type="text" name="journal_entries[{{ $counter }}][debtor]" value="{{ $debtor }}" class="form-control input_number debtor" required>
                            </td>

                            <td>
                                <input type="text" name="journal_entries[{{ $counter }}][creditor]" value="{{ $creditor }}" class="form-control input_number creditor" required>
                            </td>
                            ...
                            <td>
                                <select name="journal_entries[{{ $counter }}][cost_cen_field_id]" class="form-control select2 cost_cen_field_id" style="width: 100%;" placeholder="Select Addtional Cost Center">
                                    <option value="">{{ __('Select Addtional Cost Center') }}</option>
                                    @foreach($ac_cost_cen_field_adds as $id => $desc)
                                    <option value="{{ $id }}" {{ $entry->cost_cen_field_id == $id ? 'selected' : '' }}>{{ $desc }}</option>
                                    @endforeach
                                </select>
                            </td>
                    ...
                    <tbody>
                        ...
                        <tr style="background-color: greenyellow">
                            <td class="text-center"><span>المجموع :</span> <span
                                    id="total_debtor">{{ $total_debtor }}</span> </td>
                            <td> <span>المجموع :</span> <span id="total_creditor"> {{ $total_creditor }}</span></td>
    ...
    $(document).ready(function() {
        ...
        function calculateDebtorCreditorTotalMain() {
            ...
            if (total_creditor != total_debtor) {
                $("#unbalanced_entry_value_dif").text(Math.abs(total_creditor - total_debtor));
