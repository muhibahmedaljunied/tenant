			...
			<div class="form-group">
				...
				<select name="location_id" id="select_location_id" class="form-control" required>
					...
					@foreach($business_locations as $key => $value)
						<option value="{{ $key }}" @if($default_location == $key) selected @endif>{{ $value }}</option>
					@endforeach
				</select>
			</div>
		</div>
					...
					<div class="col-sm-8 col-sm-offset-2">
						<div class="form-group">
							<div class="input-group">
								...
								<input type="text" name="search_product" id="search_product" class="form-control" placeholder="{{ __('lang_v1.search_product_placeholder') }}" @if(is_null($default_location)) disabled @else autofocus @endif>
							</div>
						</div>
					</div>
				</div>
