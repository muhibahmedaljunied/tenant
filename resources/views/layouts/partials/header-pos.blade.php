    ...
    <div class="row">
                    ...
                    @if (empty($transaction->location_id))
                        @if (count($business_locations) > 1)
                            <div style="width: 28%;margin-bottom: 5px;">
                                <select name="select_location_id" id="select_location_id" class="form-control input-sm"
                                    required autofocus
                                    @foreach ($bl_attributes as $attr => $val)
                            {{ $attr }}="{{ is_array($val) ? json_encode($val) : $val }}" @endforeach>
                                    @foreach ($business_locations as $id => $name)
                                        <option value="{{ $id }}"
                                            @if (($default_location->id ?? null) == $id) selected @endif>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <div class="col-md-6">
            ...
            @can('access_sell_return')
                <a class="btn btn-primary btn-flat m-6 btn-xs m-5 pull-right" target="_blank"
                    href="{{ URL::to('/reports/product-sell-return-report') }}">
