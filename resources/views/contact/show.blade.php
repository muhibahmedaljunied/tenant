    ...
    <section class="content no-print">
            ...
            <div class="col-md-4 col-xs-12 mt-15 pull-right">
                <select class="form-control select2" id="contact_id" name="contact_id">
                    @foreach ($contact_dropdown as $id => $name)
                        <option value="{{ $id }}" {{ $id == optional($contact)->id ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="hide print_table_part">
            ...
            <div style="width: 100%;">
                ...
                @if (isset($contact) && $contact->type != 'customer')
                    <div class="info_col">
        ...
        <input type="hidden" id="sell_list_filter_customer_id" value="{{ optional($contact)->id }}">
        <input type="hidden" id="purchase_list_filter_supplier_id" value="{{ optional($contact)->id }}">
            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs nav-justified">
                        ...
                        @if (in_array(optional($contact)->type, ['customer', 'both']) && session('business.enable_rp'))
                            <li
                                class="
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="purchase_list_filter_date_range"
                                                name="purchase_list_filter_date_range" class="form-control"
                                                placeholder="@lang('lang_v1.select_a_date_range')" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        ...
                                        <div class="col-md-3">
                                            <div class="info-box bg-yellow">
                                                ...
                                                <div class="info-box-content">
                                                    ...
                                                    <span
                                                        class="info-box-number">{{ optional($contact)->total_rp ?? 0 }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#contact_id').change(function() {
                ...
                if (contactId) {
                    window.location = "{{ url('/contacts') }}/" + contactId;
                }
            });
        ...
        function get_contact_payments(url = null) {
            if (!url) {
                url = "{{ action('ContactController@getContactPayments', [optional($contact)->id ?? '']) }}";
            }
            $.ajax({
                url: url,
                dataType: 'html',
        ...
        function get_contact_ledger() {
            ...
            $.ajax({
                url: '/contacts/ledger?contact_id={{ optional($contact)->id ?? '' }}&start_date=' + start_date +
        ...
        $(document).on('click', '#send_ledger', function() {
            ...
            var url =
                "{{ action('NotificationController@getTemplate', [optional($contact)->id ?? '', 'send_ledger']) }}" +
...
@include('sale_pos.partials.subscriptions_table_javascript', ['contact_id' => optional($contact)->id])
