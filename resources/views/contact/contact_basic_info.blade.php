

<h3 class="profile-username">
    <i class="fas fa-user-tie"></i>
    {{ optional($contact)->name }}
    <small>
        @if(optional($contact)->type == 'both')
            {{ __('role.customer') }} & {{ __('role.supplier') }}
        @elseif(optional($contact)->type != 'lead')
            {{ __('role.' . optional($contact)->type) }}
        @endif
    </small>
</h3>



<br>
<strong><i class="fa fa-map-marker margin-r-5"></i> @lang('business.address')</strong>
<p class="text-muted">
    {!! optional($contact)->contact_address !!}
</p>

@if(optional($contact)->supplier_business_name)
    <strong><i class="fa fa-briefcase margin-r-5"></i> 
    @lang('business.business_name')</strong>
    <p class="text-muted">
        {{ optional($contact)->supplier_business_name }}
    </p>
@endif

@if(optional($contact)->mobile)
    <strong><i class="fa fa-mobile margin-r-5"></i> @lang('contact.mobile')</strong>
    <p class="text-muted">
        {{ optional($contact)->mobile }}
    </p>
@endif

@if(optional($contact)->landline)
    <strong><i class="fa fa-phone margin-r-5"></i> @lang('contact.landline')</strong>
    <p class="text-muted">
        {{ optional($contact)->landline }}
    </p>
@endif

@if(optional($contact)->alternate_number)
    <strong><i class="fa fa-phone margin-r-5"></i> @lang('contact.alternate_contact_number')</strong>
    <p class="text-muted">
        {{ optional($contact)->alternate_number }}
    </p>
@endif

@if(optional($contact)->dob)
    <strong><i class="fa fa-calendar margin-r-5"></i> @lang('lang_v1.dob')</strong>
    <p class="text-muted">
        {{ @format_date(optional($contact)->dob) }}
    </p>
@endif
