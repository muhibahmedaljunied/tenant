...
<body>
    ...
    {
    $fmt = datefmt_create('ar_EG', IntlDateFormatter::FULL, IntlDateFormatter::FULL, 'Africa/Cairo',
    IntlDateFormatter::GREGORIAN, 'yyyy / M / d');
    ...
    <div style="page-break-after: always;">
        ...
        <table class="table table-bordered" id="installmentTable">
            ...
            <tr>
                ...
                <td width="99%">
                    {{ arToIn(collect($transaction->payment_lines)->sum('amount'), 'DECIMAL') }}
                </td>
            </tr>
            <tr>
                <td style="white-space: nowrap;">المبلغ المتبقى</td>
                <td width="99%">
                    {{ arToIn($transaction->final_total -
                    collect($transaction->installment)->where('status',0)->sum('amount'), 'DECIMAL') }}
                </td>
            </tr>
            <tr>
        ...
        <table id="installment_calculations">
            ...
            <tr>
                ...
                <td style="width: 99%;">
                    ...
                    {{ arToIn($transaction->installmentBenefit->installment_amount +
                    $transaction->installmentBenefit->benefits - $payment_lines_total, 'DECIMAL') }}
                </td>
            </tr>
        ...
        <table class="table table-bordered">
            ...
            @foreach ($transaction->installment as $installment)
                    <tr>
                        ...
                        <td>{{ dateFormate($installment->due_date) }}</td>
                        <td>{{ arToIn($installment->amount, 'DECIMAL') }}</td>
                        <td style="text-align: center;">{{ $installment->status == 1 ? "&#10003;" : '-' }}</td>
                    </tr>
            @endforeach
        </table>
        @endif
    ...
    @if (count($transaction->installment))
    <div style="page-break-after: always;width: calc(100% - 100px);margin:auto;" id="commodity-sale-statement">
        ...
        <p>المبلغ المتبقي /
            ...
            {{ arToIn($transaction->installmentBenefit->installment_amount + $transaction->installmentBenefit->benefits
            - $payment_lines_total, 'DECIMAL') }}
        </p>
        <br>
    ...
    <div class="ticket">
        ...
        @if($receipt_details->show_qr_code && !empty($receipt_details->qr_code_gen))
        <img class="center-block mt-5"
            src="data:image/png;base64,{{DNS2D::getBarcodePNG($receipt_details->qr_code_gen, 'QRCODE')}}">
        @endif
