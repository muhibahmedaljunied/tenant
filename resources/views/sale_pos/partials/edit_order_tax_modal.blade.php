					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<div class="input-group">
							  ...
							  <select name="order_tax_modal" id="order_tax_modal" class="form-control"
								@foreach($taxes['attributes'] ?? [] as $attr => $val)
								  {{ $attr }}="{{ is_array($val) ? json_encode($val) : $val }}"
								@endforeach
							  >
								<option value="">{{ __('messages.please_select') }}</option>
								@foreach($taxes['tax_rates'] as $key => $rate)
								  <option value="{{ $key }}" @if($selected_tax == $key) selected @endif>
									{{ $rate }}
								  </option>
								@endforeach
							  </select>
