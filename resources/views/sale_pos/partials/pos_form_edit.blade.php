...
<div class="row">
		...
		<div class="form-group">
			<select name="invoice_layout_id" id="invoice_layout_id" class="form-control select2">
				...
				@foreach($invoice_layouts as $key => $value)
					<option value="{{ $key }}" @if($transaction->location->invoice_layout_id == $key) selected @endif>{{ $value }}</option>
				@endforeach
			</select>
		</div>
	</div>
		...
		<div class="col-md-4 col-sm-6">
			<div class="form-group">
				<div class="input-group">
					...
					<input type="text" name="transaction_date" id="transaction_date" class="form-control" value="{{ @format_datetime($transaction->transaction_date) }}" readonly required>
				</div>
			</div>
		</div>
	@endif
		...
		<div class="col-md-4 col-sm-6">
			<div class="form-group">
				<div class="input-group">
					...
					<input type="text" name="exchange_rate" id="exchange_rate" class="form-control input-sm input_number" placeholder="{{ __('lang_v1.currency_exchange_rate') }}" value="{{ @num_format($transaction->exchange_rate) }}">
				</div>
			</div>
		</div>
	@endif
			...
			<div class="form-group">
				<select name="invoice_scheme_id" class="form-control">
					...
					@foreach($invoice_schemes as $key => $value)
						<option value="{{ $key }}" @if($default_invoice_schemes->id == $key) selected @endif>{{ $value }}</option>
					@endforeach
				</select>
			</div>
		</div>
    ...
    @if(in_array('subscription', $enabled_modules))
		<div class="col-md-4 col-sm-6">
			<label>
              <input type="checkbox" name="is_recurring" id="is_recurring" class="input-icheck" value="1" @if($transaction->is_recurring) checked @endif> @lang('lang_v1.subscribe')?
