...
<section class="content no-print">
    ...
    <input type="hidden" name="location_id" id="location_id" value="{{ $transaction->location_id }}" data-receipt_printer_type="{{ !empty($location_printer_type) ? $location_printer_type : 'browser' }}" data-default_accounts="{{ $transaction->location->default_payment_accounts }}">
				<!-- /.box-header -->
				<div class="box-body">
					<div class="row">
						...
						<div class="col-sm-4">
							...
							<select name="commission_agent" class="form-control select2">
							  ...
							  @foreach($commission_agent as $key => $value)
							    <option value="{{ $key }}" {{ $transaction->commission_agent == $key ? 'selected' : '' }}>{{ $value }}</option>
							  @endforeach
							</select>
							</div>
						</div>
