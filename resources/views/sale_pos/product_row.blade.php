...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        @if (session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
            @php
                ...
                $patch_number_enabled = (bool) session()->get('business.enable_patch_number');
                $lot_no_line_id = '';
                ...
                <select class="form-control lot_number input-sm" name="products[{{ $row_count }}][lot_no_line_id]"
                    ...
                    @foreach ($product->lot_numbers as $lot_number)
                        ...
                        <option value="{{ $lot_number->purchase_line_id }}"
                            ...
                            @endif
                            @if ($patch_number_enabled && !empty($lot_number->patch_number))
                                | {{ $lot_number->patch_number }}
                            @endif
    ...
    <td>
        ...
        @if (empty($multi_units))
            @if (count($sub_units) > 0 && empty($product->parent_varId))
                <br>
            ...
            @endif
        @else
            @include('sale_pos.partials.multi_unit')
        @endif
    ...
    @if (!empty($is_direct_sell))
            ...
            <td>
                <div class="form-group">
                    <div class="input-group">
                        <select name="products[{{ $row_count }}][res_service_staff_id]" 
                                class="form-control select2 order_line_service_staff"
                                @if(!empty($pos_settings['is_service_staff_required']) && $pos_settings['is_service_staff_required'] == 1) required @endif>
                            <option value="">{{ __('restaurant.select_service_staff') }}</option>
                            @foreach($waiters as $id => $name)
                                <option value="{{ $id }}" 
                                    @if(!empty($product->res_service_staff_id) && $product->res_service_staff_id == $id) selected @endif>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <td @if (!$edit_discount) hide @endif>
            <input type="text" 
                   ...
                   class="form-control input_number row_discount_amount"><br>
        ...
        <td class="text-center {{ $hide_tax }}">
            <input type="hidden" name="products[{{ $row_count }}][item_tax]" value="{{ @num_format($item_tax) }}" class="item_tax">
            <select name="products[{{ $row_count }}][tax_id]" class="form-control tax_id">
                <option value="">{{ __('messages.please_select') }}</option>
                @foreach($tax_dropdown['tax_rates'] as $id => $name)
                    <option value="{{ $id }}"
                        @if($tax_id == $id) selected @endif
                        @if(isset($tax_dropdown['attributes'][$id]))
                            @foreach($tax_dropdown['attributes'][$id] as $attr => $val)
                                {{ $attr }}="{{ $val }}"
                            @endforeach
                        @endif
                    >{{ $name }}</option>
        ...
        @if (!empty($warranties))
            <select name="products[{{ $row_count }}][warranty_id]" class="form-control">
                ...
                @foreach($warranties as $id => $name)
                    <option value="{{ $id }}" @if($warranty_id == $id) selected @endif>{{ $name }}</option>
                @endforeach
            </select>
        @endif
    ...
    <td class="{{ $hide_tax }}">
        ...
        <input type="hidden" name="products[{{ $row_count }}][default_purchase_price]"
            class="form-control default_purchase_price input_number"
