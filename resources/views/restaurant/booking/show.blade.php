	...
	<div class="modal-content">
		<div class="modal-header">
			...
			<div class="modal-body">
				...
				<div class="row">
					<div class="col-sm-9">
						<form method="POST" action="{{ action('Restaurant\BookingController@update', [$booking->id]) }}" id="edit_booking_form">
							@csrf
							@method('PUT')
							<div class="input-group">
								<select name="booking_status" class="form-control" required>
									<option value="">{{ __('restaurant.change_booking_status') }}</option>
									@foreach($booking_statuses as $key => $value)
										<option value="{{ $key }}" @if($booking->booking_status == $key) selected @endif>{{ $value }}</option>
									@endforeach
								</select>
