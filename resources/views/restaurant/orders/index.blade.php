...
@section('content')
    ...
    <section class="content min-height-90hv no-print">
        ...
        <div class="row">
            ...
            <div class="col-sm-12">
                <button type="button" class="btn btn-sm btn-primary pull-right" id="refresh_orders"><i class="fas fa-sync"></i>
                    @lang('restaurant.refresh')</button>
            </div>
        </div>
        <br>
        <div class="row">
            @if (!$is_service_staff)
                @component('components.widget')
                    <div class="col-sm-6">
                        <form action="{{ action('Restaurant\OrderController@index') }}" method="get"
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    ...
                                    <select id="service_staff_id" name="service_staff" class="form-control select2">
                                        <option value="">{{ __('restaurant.select_service_staff') }}</option>
                                        @foreach ($service_staff as $key => $value)
                                            <option value="{{ $key }}"
                                                {{ request()->service_staff == $key ? 'selected' : '' }}>{{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
