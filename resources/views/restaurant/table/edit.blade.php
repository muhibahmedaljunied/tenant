    ...
    <form action="{{ action('Restaurant\\TableController@update', [$table->id]) }}" method="POST" id="table_edit_form">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input type="text" name="name" id="name" class="form-control" required placeholder="{{ __( 'brand.brand_name' ) }}" value="{{ $table->name }}">
        </div>

        <div class="form-group">
          <label for="description">{{ __( 'restaurant.short_description' ) . ':' }}</label>
          <input type="text" name="description" id="description" class="form-control" placeholder="{{ __( 'brand.short_description' ) }}" value="{{ $table->description }}">
        </div>
      </div>
