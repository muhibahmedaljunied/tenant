        ...
        <p ><strong>@lang('sale.location'): &nbsp;</strong> 
          @if(empty($transaction->location_id))
            ...
            <div style="width: 28%;margin-bottom: 5px;">
               <select name="select_location_id" id="select_location_id" class="form-control input-sm" required autofocus @if(isset($bl_attributes)) @foreach($bl_attributes as $attr => $val) {{ $attr }}="{{ $val }}" @endforeach @endif>
                  @foreach($business_locations as $key => $value)
                    <option value="{{ $key }}" @if(($default_location->id ?? null) == $key) selected @endif>{{ $value }}</option>
                  @endforeach
               </select>
            </div>
            @else
