<div class="modal-dialog" role="document">
  <div class="modal-content">
    <form action="{{ action('TransactionPaymentController@postPayContactDue') }}" method="post" id="pay_contact_due_form" enctype="multipart/form-data">
      @csrf
      ...
      <div class="modal-body">
        ...
        <div class="row payment_row">
          <div class="col-md-4">
            <div class="form-group">
              ...
              <div class="input-group">
                ...
                <input type="text" name="amount" id="amount" class="form-control input_number" required placeholder="{{ __('sale.amount') }}" value="{{ @num_format($payment_line->amount) }}" data-rule-max-value="{{ $payment_line->amount }}" data-msg-max-value="{{ __('lang_v1.max_amount_to_be_paid_is', ['amount' => $amount_formated]) }}">
                @else
                  <input type="text" name="amount" id="amount" class="form-control input_number" required placeholder="{{ __('sale.amount') }}" value="{{ @num_format($payment_line->amount) }}">
                @endif
              </div>
            </div>
          </div>
          ...
          <div class="col-md-12">
            <div class="row">
              @if(!empty($branchCenters))
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="branch_id">مركز تكلفة الفرع ---:</label>
                    <div class="input-group">
                      <span class="input-group-addon">
                      ...
                      <select name="branch_id" id="branch_id" class="form-control select2" style="width:100%;">
                        <option value="">--</option>
                        @foreach($branchCenters as $key => $value)
                          <option value="{{ $key }}" {{ (!empty($payment_line->account_id) && $payment_line->account_id == $key) ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                      </select>
                    </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="extra_branch_id"># مركز تكلفة اضافيه:</label>
                    <div class="input-group">
                      <span class="input-group-addon">
                        <i class="fas fa-code-branch"></i>
                      </span>
                      <select name="extra_branch_id" id="extra_branch_id" class="form-control select2" style="width:100%;">
                        <option value="">--</option>
                        @foreach($extraCenters as $key => $value)
                          <option value="{{ $key }}" {{ (!empty($payment_line->account_id) && $payment_line->account_id == $key) ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                      </select>
                    </div>
