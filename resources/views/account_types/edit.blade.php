<div class="modal-dialog" role="document">
    ...
    <div class="modal-body">
          ...
          <input type="text" name="name" id="name" class="form-control" required placeholder="{{ __( 'lang_v1.name' ) }}" value="{{ $account_type->name }}">
        </div>
      ...
      <div class="form-group">
          ...
          <select name="parent_account_type_id" id="parent_account_type_id" class="form-control">
            ...
            @foreach($account_types as $type)
              <option value="{{ $type->id }}" {{ $account_type->parent_account_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
            @endforeach
          </select>
      </div>
    </div>
