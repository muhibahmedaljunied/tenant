@extends('layouts.app')
@section('title', __('chart_of_accounts.edit_cost_center_field_add'))

@section('content')

    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>@lang('chart_of_accounts.edit_cost_center_field_add')</h1>
        <!-- <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        @endphp
        <form action="{{ action('AcCostCenFieldAddController@update', [$ac_cost_details->id]) }}" method="POST" id="cost_center_field_add_edit_form" class="cost_center_field_add_form{{ $form_class }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="users[]" id="users" class="form-control select2" multiple>
                            @foreach($users as $key => $value)
                                <option value="{{ $key }}" {{ (collect(old('users', $ac_cost_details->ac_cost_cen_field_adds->pluck('id')))->contains($key)) ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
