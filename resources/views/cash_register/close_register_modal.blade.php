        ...
        <div class="col-sm-4">
          <div class="form-group">
            <label for="closing_amount">{{ __( 'cash_register.total_cash' ) . ':*' }}</label>
              <input type="text" name="closing_amount" id="closing_amount" value="{{ @num_format($register_details->cash_in_hand + $register_details->total_cash - $register_details->total_cash_refund) }}" class="form-control input_number" required placeholder="{{ __( 'cash_register.total_cash' ) }}">
          </div>
        </div>
        <div class="col-sm-4">
          <div class="form-group">
            <label for="total_card_slips">{{ __( 'cash_register.total_card_slips' ) . ':*' }}</label> @show_tooltip(__('tooltip.total_card_slips'))
              <input type="number" name="total_card_slips" id="total_card_slips" value="{{ $register_details->total_card_slips }}" class="form-control" required placeholder="{{ __( 'cash_register.total_card_slips' ) }}" min="0">
          </div>
        </div> 
        <div class="col-sm-4">
          <div class="form-group">
            <label for="total_cheques">{{ __( 'cash_register.total_cheques' ) . ':*' }}</label> @show_tooltip(__('tooltip.total_cheques'))
              <input type="number" name="total_cheques" id="total_cheques" value="{{ $register_details->total_cheques }}" class="form-control" required placeholder="{{ __( 'cash_register.total_cheques' ) }}" min="0">
          </div>
        </div> 
        <div class="col-sm-12">
          <div class="form-group">
            <label for="closing_note">{{ __( 'cash_register.closing_note' ) . ':' }}</label>
              <textarea name="closing_note" id="closing_note" class="form-control" placeholder="{{ __( 'cash_register.closing_note' ) }}" rows="3"></textarea>
          </div>
        </div>
      </div> 
      <div class="row">
