<tr class="product_row">
    <td>
        ...
        {{$product->related_variations->sub_sku ?? $product->sub_sku}}
    </td>
    ...
    <td>
        ...
        <input type="hidden" value="{{$product->parent_varId}}" name="products[{{$row_index}}][parent_variation_id]">

        <input type="hidden" value="{{$product->related_variations->id?? $product->variation_id}}" 
        ...
        @if(!empty($edit))
            ...
            @php
                $qty = $product->quantity_returned / ($product->related_variations->equal_qty ?? 1);
        ...
        data-msg_max_default="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $product->formatted_qty_available, 'unit' => $product->unit  ])"
         @endif >
        {{$product->v_unit ?? $product->related_variations->unit->short_name ?? $product->unit}}
    </td>
    <td>
